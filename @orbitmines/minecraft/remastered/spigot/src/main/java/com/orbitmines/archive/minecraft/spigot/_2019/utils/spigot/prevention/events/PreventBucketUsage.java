...
public class PreventBucketUsage extends PreventionEvent<PlayerInteractEvent> {

    public PreventBucketUsage(World world) {
        super(world);
    }

    @Override
    protected boolean shouldBeCancelled(PlayerInteractEvent event, World world) {
        return event.getAction() == Action.RIGHT_CLICK_BLOCK && ItemUtils.isBucket(event.getItem());
    }
