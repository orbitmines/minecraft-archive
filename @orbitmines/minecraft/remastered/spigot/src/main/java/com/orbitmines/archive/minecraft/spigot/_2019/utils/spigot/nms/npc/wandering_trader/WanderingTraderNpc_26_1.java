package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.wandering_trader;
...
public class WanderingTraderNpc_26_1 implements WanderingTraderNpc {

    private final NpcNms npcNms;

    public WanderingTraderNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomWanderingTraderNpc entity = new CustomWanderingTraderNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftWanderingTrader) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomWanderingTraderNpc extends WanderingTrader {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomWanderingTraderNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomWanderingTraderNpc(Level level, Collection<Option> options, NpcNms npcNms) {
