package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.hoglin;
...
public class HoglinNpc_26_1 implements HoglinNpc {

    private final NpcNms npcNms;

    public HoglinNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomHoglinNpc entity = new CustomHoglinNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftHoglin) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomHoglinNpc extends Hoglin {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomHoglinNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomHoglinNpc(Level level, Collection<Option> options, NpcNms npcNms) {
