package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.pufferfish;
...
public class PufferFishNpc_1_14_R1 implements PufferFishNpc {

    private final NpcNms npcNms;

    public PufferFishNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomPufferFishNpc entity = new CustomPufferFishNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftPufferFish) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomPufferFishRideable entity = new CustomPufferFishRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomPufferFishNpc extends EntityPufferFish {

        private final Collection<Option> options;

        CustomPufferFishNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomPufferFishNpc(World world, Collection<Option> options) {
...
//    private static class CustomPufferFishRideable extends EntityPufferFish {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomPufferFishRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomPufferFishRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
