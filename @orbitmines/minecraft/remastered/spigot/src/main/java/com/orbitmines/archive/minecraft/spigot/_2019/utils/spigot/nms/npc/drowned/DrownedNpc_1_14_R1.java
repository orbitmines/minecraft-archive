...
public class DrownedNpc_1_14_R1 implements DrownedNpc {

    private final NpcNms npcNms;

    public DrownedNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomDrownedNpc entity = new CustomDrownedNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftDrowned) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomDrownedRideable entity = new CustomDrownedRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomDrownedNpc extends EntityDrowned {

        private final Collection<Option> options;

        CustomDrownedNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomDrownedNpc(World world, Collection<Option> options) {
...
//    private static class CustomDrownedRideable extends EntityDrowned {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomDrownedRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomDrownedRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
