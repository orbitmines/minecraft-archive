package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.camel_husk;
...
public class CamelHuskNpc_26_1 implements CamelHuskNpc {

    private final NpcNms npcNms;

    public CamelHuskNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomCamelHuskNpc entity = new CustomCamelHuskNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftCamelHusk) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomCamelHuskNpc extends CamelHusk {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomCamelHuskNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomCamelHuskNpc(Level level, Collection<Option> options, NpcNms npcNms) {
