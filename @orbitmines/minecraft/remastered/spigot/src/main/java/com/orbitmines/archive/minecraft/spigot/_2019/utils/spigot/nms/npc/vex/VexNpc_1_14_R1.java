package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.vex;
...
public class VexNpc_1_14_R1 implements VexNpc {

    private final NpcNms npcNms;

    public VexNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomVexNpc entity = new CustomVexNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftVex) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomVexRideable entity = new CustomVexRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomVexNpc extends EntityVex {

        private final Collection<Option> options;

        CustomVexNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomVexNpc(World world, Collection<Option> options) {
...
//    private static class CustomVexRideable extends EntityVex {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomVexRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomVexRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
