package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.pig;
...
public class PigNpc_1_14_R1 implements PigNpc {

    private final NpcNms npcNms;

    public PigNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomPigNpc entity = new CustomPigNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftPig) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomPigRideable entity = new CustomPigRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomPigNpc extends EntityPig {

        private final Collection<Option> options;

        CustomPigNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomPigNpc(World world, Collection<Option> options) {
...
//    private static class CustomPigRideable extends EntityPig {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomPigRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomPigRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
