package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.squid;
...
public class SquidNpc_1_14_R1 implements SquidNpc {

    private final NpcNms npcNms;

    public SquidNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomSquidNpc entity = new CustomSquidNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftSquid) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomSquidRideable entity = new CustomSquidRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomSquidNpc extends EntitySquid {

        private final Collection<Option> options;

        CustomSquidNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomSquidNpc(World world, Collection<Option> options) {
...
//
//    private static class CustomSquidRideable extends EntitySquid {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomSquidRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomSquidRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
