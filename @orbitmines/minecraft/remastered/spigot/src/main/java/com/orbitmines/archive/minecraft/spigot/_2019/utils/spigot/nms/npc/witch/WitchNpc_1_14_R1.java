package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.witch;
...
public class WitchNpc_1_14_R1 implements WitchNpc {

    private final NpcNms npcNms;

    public WitchNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomWitchNpc entity = new CustomWitchNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftWitch) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomWitchRideable entity = new CustomWitchRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomWitchNpc extends EntityWitch {

        private final Collection<Option> options;

        CustomWitchNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomWitchNpc(World world, Collection<Option> options) {
...
//    private static class CustomWitchRideable extends EntityWitch {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomWitchRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomWitchRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
