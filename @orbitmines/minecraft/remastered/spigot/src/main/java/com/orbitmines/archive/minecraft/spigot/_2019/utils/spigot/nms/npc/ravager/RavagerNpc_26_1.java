package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.ravager;
...
public class RavagerNpc_26_1 implements RavagerNpc {

    private final NpcNms npcNms;

    public RavagerNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomRavagerNpc entity = new CustomRavagerNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftRavager) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomRavagerNpc extends Ravager {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomRavagerNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomRavagerNpc(Level level, Collection<Option> options, NpcNms npcNms) {
