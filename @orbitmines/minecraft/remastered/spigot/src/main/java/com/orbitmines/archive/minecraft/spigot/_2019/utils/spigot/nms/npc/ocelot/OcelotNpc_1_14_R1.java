package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.ocelot;
...
public class OcelotNpc_1_14_R1 implements OcelotNpc {

    private final NpcNms npcNms;

    public OcelotNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomOcelotNpc entity = new CustomOcelotNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftOcelot) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomOcelotRideable entity = new CustomOcelotRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomOcelotNpc extends EntityOcelot {

        private final Collection<Option> options;

        CustomOcelotNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomOcelotNpc(World world, Collection<Option> options) {
...
//    private static class CustomOcelotRideable extends EntityOcelot {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomOcelotRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomOcelotRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
