package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.glow_squid;
...
public class GlowSquidNpc_26_1 implements GlowSquidNpc {

    private final NpcNms npcNms;

    public GlowSquidNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomGlowSquidNpc entity = new CustomGlowSquidNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftGlowSquid) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomGlowSquidNpc extends GlowSquid {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomGlowSquidNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomGlowSquidNpc(Level level, Collection<Option> options, NpcNms npcNms) {
            super(EntityType.GLOW_SQUID, level);
