package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.rabbit;
...
public class RabbitNpc_1_14_R1 implements RabbitNpc {

    private final NpcNms npcNms;

    public RabbitNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomRabbitNpc entity = new CustomRabbitNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftRabbit) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomRabbitRideable entity = new CustomRabbitRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomRabbitNpc extends EntityRabbit {

        private final Collection<Option> options;

        CustomRabbitNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomRabbitNpc(World world, Collection<Option> options) {
...
//    private static class CustomRabbitRideable extends EntityRabbit {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomRabbitRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomRabbitRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
