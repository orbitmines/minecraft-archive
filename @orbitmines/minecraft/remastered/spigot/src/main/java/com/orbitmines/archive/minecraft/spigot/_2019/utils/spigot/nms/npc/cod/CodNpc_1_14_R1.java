package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.cod;
...
public class CodNpc_1_14_R1 implements CodNpc {

    private final NpcNms npcNms;

    public CodNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomCodNpc entity = new CustomCodNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftCod) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomCodRideable entity = new CustomCodRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomCodNpc extends EntityCod {

        private final Collection<Option> options;

        CustomCodNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomCodNpc(World world, Collection<Option> options) {
...
//    private static class CustomCodRideable extends EntityCod {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomCodRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomCodRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
