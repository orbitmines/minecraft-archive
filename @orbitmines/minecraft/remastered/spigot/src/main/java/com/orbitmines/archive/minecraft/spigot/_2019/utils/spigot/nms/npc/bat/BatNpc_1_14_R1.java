package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.bat;
...
public class BatNpc_1_14_R1 implements BatNpc {

    private final NpcNms npcNms;

    public BatNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomBatNpc entity = new CustomBatNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftBat) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomBatRideable entity = new CustomBatRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomBatNpc extends EntityBat {

        private final Collection<Option> options;

        CustomBatNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomBatNpc(World world, Collection<Option> options) {
...
//    private static class CustomBatRideable extends EntityBat {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomBatRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomBatRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
//            super(EntityTypes.BAT, world);
//
//            this.npcNms = npcNms;
//
