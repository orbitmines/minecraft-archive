package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.guardian;
...
public class GuardianNpc_1_13_R2 implements GuardianNpc {

    private final NpcNms npcNms;

    public GuardianNpc_1_13_R2(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomGuardianNpc entity = new CustomGuardianNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftGuardian) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomGuardianRideable entity = new CustomGuardianRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomGuardianNpc extends EntityGuardian {

        private final Collection<Option> options;

        CustomGuardianNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomGuardianNpc(World world, Collection<Option> options) {
...
//    private static class CustomGuardianRideable extends EntityGuardian {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomGuardianRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomGuardianRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
