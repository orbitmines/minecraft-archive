package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.zoglin;
...
public class ZoglinNpc_26_1 implements ZoglinNpc {

    private final NpcNms npcNms;

    public ZoglinNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomZoglinNpc entity = new CustomZoglinNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftZoglin) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomZoglinNpc extends Zoglin {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomZoglinNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomZoglinNpc(Level level, Collection<Option> options, NpcNms npcNms) {
