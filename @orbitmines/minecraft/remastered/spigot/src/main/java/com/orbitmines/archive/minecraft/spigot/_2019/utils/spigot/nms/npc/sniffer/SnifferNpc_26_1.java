package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.sniffer;
...
public class SnifferNpc_26_1 implements SnifferNpc {

    private final NpcNms npcNms;

    public SnifferNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomSnifferNpc entity = new CustomSnifferNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftSniffer) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomSnifferNpc extends Sniffer {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomSnifferNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomSnifferNpc(Level level, Collection<Option> options, NpcNms npcNms) {
            super(EntityType.SNIFFER, level);
