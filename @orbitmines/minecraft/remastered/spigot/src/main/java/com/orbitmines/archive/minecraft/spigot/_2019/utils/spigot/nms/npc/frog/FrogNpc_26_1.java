package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.frog;
...
public class FrogNpc_26_1 implements FrogNpc {

    private final NpcNms npcNms;

    public FrogNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomFrogNpc entity = new CustomFrogNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftFrog) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomFrogNpc extends Frog {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomFrogNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomFrogNpc(Level level, Collection<Option> options, NpcNms npcNms) {
