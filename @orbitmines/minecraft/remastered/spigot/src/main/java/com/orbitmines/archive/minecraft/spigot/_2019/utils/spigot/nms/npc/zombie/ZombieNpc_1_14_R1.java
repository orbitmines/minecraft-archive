package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.zombie;
...
public class ZombieNpc_1_14_R1 implements ZombieNpc {

    private final NpcNms npcNms;

    public ZombieNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomZombieNpc entity = new CustomZombieNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftZombie) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomZombieRideable entity = new CustomZombieRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomZombieNpc extends EntityZombie {

        private final Collection<Option> options;

        CustomZombieNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomZombieNpc(World world, Collection<Option> options) {
...
//    private static class CustomZombieRideable extends EntityZombie {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomZombieRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomZombieRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
