...
public class CommandTeleportHere extends Command<Survival, SurvivalPlayer> {

    public CommandTeleportHere(Survival plugin) {
        super(plugin, Server.SURVIVAL, "tphere", "tph", "teleporthere");
        ...
        withArg(
            new PlayerArgument<Survival, SurvivalPlayer>(plugin, false).executes((Executor1<Survival, SurvivalPlayer,
                ...
                player.sendMessage("Teleport", Color.LIME, "survival", "player.command.tphere.sent", target.getName(Name.RAW_COLORED) + "§7");

                target.sendRawMessage("");
                target.sendMessage("Teleport", Color.BLUE, "survival", "player.command.tphere.received", player.getName(Name.RAW_COLORED) + "§7");
        ...
        requires(VipRank.EMERALD);
    }

    @Override
    public String getDescription(SurvivalPlayer player) {
        return player.translate("survival", "player.command.tphere.description");
    }
}
