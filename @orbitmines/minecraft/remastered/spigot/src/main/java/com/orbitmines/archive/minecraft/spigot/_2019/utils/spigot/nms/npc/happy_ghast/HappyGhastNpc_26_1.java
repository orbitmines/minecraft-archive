package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.happy_ghast;
...
public class HappyGhastNpc_26_1 implements HappyGhastNpc {

    private final NpcNms npcNms;

    public HappyGhastNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomHappyGhastNpc entity = new CustomHappyGhastNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftHappyGhast) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomHappyGhastNpc extends HappyGhast {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomHappyGhastNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomHappyGhastNpc(Level level, Collection<Option> options, NpcNms npcNms) {
