package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.illusioner;
...
public class IllusionerNpc_1_14_R1 implements IllusionerNpc {

    private final NpcNms npcNms;

    public IllusionerNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomIllagerIllusionerNpc entity = new CustomIllagerIllusionerNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftIllusioner) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomIllagerIllusionerRideable entity = new CustomIllagerIllusionerRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomIllagerIllusionerNpc extends EntityIllagerIllusioner {

        private final Collection<Option> options;

        CustomIllagerIllusionerNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomIllagerIllusionerNpc(World world, Collection<Option> options) {
...
//    private static class CustomIllagerIllusionerRideable extends EntityIllagerIllusioner {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomIllagerIllusionerRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomIllagerIllusionerRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
