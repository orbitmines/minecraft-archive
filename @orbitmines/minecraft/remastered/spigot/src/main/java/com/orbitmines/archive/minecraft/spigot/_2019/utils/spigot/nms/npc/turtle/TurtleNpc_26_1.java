package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.turtle;
...
public class TurtleNpc_26_1 implements TurtleNpc {

    private final NpcNms npcNms;

    public TurtleNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomTurtleNpc entity = new CustomTurtleNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftTurtle) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomTurtleNpc extends Turtle {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomTurtleNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomTurtleNpc(Level level, Collection<Option> options, NpcNms npcNms) {
