package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.vindicator;
...
public class VindicatorNpc_1_14_R1 implements VindicatorNpc {

    private final NpcNms npcNms;

    public VindicatorNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomVindicatorNpc entity = new CustomVindicatorNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftVindicator) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomVindicatorRideable entity = new CustomVindicatorRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomVindicatorNpc extends EntityVindicator {

        private final Collection<Option> options;

        CustomVindicatorNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomVindicatorNpc(World world, Collection<Option> options) {
...
//    private static class CustomVindicatorRideable extends EntityVindicator {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomVindicatorRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomVindicatorRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
