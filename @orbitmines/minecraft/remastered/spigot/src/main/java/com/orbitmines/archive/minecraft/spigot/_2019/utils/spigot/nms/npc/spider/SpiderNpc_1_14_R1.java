package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.spider;
...
public class SpiderNpc_1_14_R1 implements SpiderNpc {

    private final NpcNms npcNms;

    public SpiderNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomSpiderNpc entity = new CustomSpiderNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftSpider) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomSpiderRideable entity = new CustomSpiderRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomSpiderNpc extends EntitySpider {

        private final Collection<Option> options;

        CustomSpiderNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomSpiderNpc(World world, Collection<Option> options) {
...
//    private static class CustomSpiderRideable extends EntitySpider {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomSpiderRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomSpiderRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
