package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.panda;
...
public class PandaNpc_26_1 implements PandaNpc {

    private final NpcNms npcNms;

    public PandaNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomPandaNpc entity = new CustomPandaNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftPanda) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomPandaNpc extends Panda {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomPandaNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomPandaNpc(Level level, Collection<Option> options, NpcNms npcNms) {
