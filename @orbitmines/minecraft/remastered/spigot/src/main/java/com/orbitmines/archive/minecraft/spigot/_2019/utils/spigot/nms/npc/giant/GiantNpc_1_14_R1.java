package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.giant;
...
public class GiantNpc_1_14_R1 implements GiantNpc {

    private final NpcNms npcNms;

    public GiantNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomGiantZombieNpc entity = new CustomGiantZombieNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftGiant) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomGiantZombieRideable entity = new CustomGiantZombieRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomGiantZombieNpc extends EntityGiantZombie {

        private final Collection<Option> options;

        CustomGiantZombieNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomGiantZombieNpc(World world, Collection<Option> options) {
...
//    private static class CustomGiantZombieRideable extends EntityGiantZombie {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomGiantZombieRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomGiantZombieRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
