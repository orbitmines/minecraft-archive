package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.pig_zombie;
...
public class PigZombieNpc_1_14_R1 implements PigZombieNpc {

    private final NpcNms npcNms;

    public PigZombieNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomPigZombieNpc entity = new CustomPigZombieNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftPigZombie) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomPigZombieRideable entity = new CustomPigZombieRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomPigZombieNpc extends EntityPigZombie {

        private final Collection<Option> options;

        CustomPigZombieNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomPigZombieNpc(World world, Collection<Option> options) {
...
//    private static class CustomPigZombieRideable extends EntityPigZombie {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomPigZombieRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomPigZombieRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
