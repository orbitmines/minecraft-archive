package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.armadillo;
...
public class ArmadilloNpc_26_1 implements ArmadilloNpc {

    private final NpcNms npcNms;

    public ArmadilloNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomArmadilloNpc entity = new CustomArmadilloNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftArmadillo) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomArmadilloNpc extends Armadillo {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomArmadilloNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomArmadilloNpc(Level level, Collection<Option> options, NpcNms npcNms) {
