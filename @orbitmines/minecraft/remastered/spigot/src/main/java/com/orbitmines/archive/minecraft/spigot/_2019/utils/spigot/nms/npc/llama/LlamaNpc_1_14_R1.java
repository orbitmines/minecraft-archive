package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.llama;
...
public class LlamaNpc_1_14_R1 implements LlamaNpc {

    private final NpcNms npcNms;

    public LlamaNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomLlamaNpc entity = new CustomLlamaNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftLlama) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomLlamaRideable entity = new CustomLlamaRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomLlamaNpc extends EntityLlama {

        private final Collection<Option> options;

        CustomLlamaNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomLlamaNpc(World world, Collection<Option> options) {
...
//            return options.contains(Option.DISABLE_SOUNDS) ? null : super.dB();
//        }
    }

//    private static class CustomLlamaRideable extends EntityLlama {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomLlamaRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomLlamaRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
