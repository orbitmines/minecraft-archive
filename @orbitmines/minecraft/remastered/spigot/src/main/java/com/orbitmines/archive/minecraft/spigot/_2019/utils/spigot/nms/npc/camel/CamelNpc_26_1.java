package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.camel;
...
public class CamelNpc_26_1 implements CamelNpc {

    private final NpcNms npcNms;

    public CamelNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomCamelNpc entity = new CustomCamelNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftCamel) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomCamelNpc extends Camel {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomCamelNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomCamelNpc(Level level, Collection<Option> options, NpcNms npcNms) {
            super(EntityType.CAMEL, level);
