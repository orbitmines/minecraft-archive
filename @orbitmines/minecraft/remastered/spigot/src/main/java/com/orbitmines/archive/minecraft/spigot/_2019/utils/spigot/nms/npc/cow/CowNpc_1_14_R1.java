package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.cow;
...
public class CowNpc_1_14_R1 implements CowNpc {

    private final NpcNms npcNms;

    public CowNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomCowNpc entity = new CustomCowNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftCow) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomCowRideable entity = new CustomCowRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomCowNpc extends EntityCow {

        private final Collection<Option> options;

        CustomCowNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomCowNpc(World world, Collection<Option> options) {
...
//    private static class CustomCowRideable extends EntityCow {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomCowRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomCowRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
