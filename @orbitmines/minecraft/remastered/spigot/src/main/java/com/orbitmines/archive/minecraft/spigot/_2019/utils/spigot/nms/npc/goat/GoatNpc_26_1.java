package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.goat;
...
public class GoatNpc_26_1 implements GoatNpc {

    private final NpcNms npcNms;

    public GoatNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomGoatNpc entity = new CustomGoatNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftGoat) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomGoatNpc extends Goat {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomGoatNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomGoatNpc(Level level, Collection<Option> options, NpcNms npcNms) {
