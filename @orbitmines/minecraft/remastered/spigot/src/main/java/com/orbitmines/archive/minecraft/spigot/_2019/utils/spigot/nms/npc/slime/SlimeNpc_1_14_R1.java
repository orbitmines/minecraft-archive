package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.slime;
...
public class SlimeNpc_1_14_R1 implements SlimeNpc {

    private final NpcNms npcNms;

    public SlimeNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomSlimeNpc entity = new CustomSlimeNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftSlime) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomSlimeRideable entity = new CustomSlimeRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomSlimeNpc extends EntitySlime {

        private final Collection<Option> options;

        CustomSlimeNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomSlimeNpc(World world, Collection<Option> options) {
...
//    private static class CustomSlimeRideable extends EntitySlime {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomSlimeRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomSlimeRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
