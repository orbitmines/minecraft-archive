package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.warden;
...
public class WardenNpc_26_1 implements WardenNpc {

    private final NpcNms npcNms;

    public WardenNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomWardenNpc entity = new CustomWardenNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftWarden) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomWardenNpc extends Warden {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomWardenNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomWardenNpc(Level level, Collection<Option> options, NpcNms npcNms) {
