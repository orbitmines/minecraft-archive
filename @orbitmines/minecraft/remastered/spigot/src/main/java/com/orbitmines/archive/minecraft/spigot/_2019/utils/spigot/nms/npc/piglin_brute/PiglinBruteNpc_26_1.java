package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.piglin_brute;
...
public class PiglinBruteNpc_26_1 implements PiglinBruteNpc {

    private final NpcNms npcNms;

    public PiglinBruteNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomPiglinBruteNpc entity = new CustomPiglinBruteNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftPiglinBrute) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomPiglinBruteNpc extends PiglinBrute {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomPiglinBruteNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomPiglinBruteNpc(Level level, Collection<Option> options, NpcNms npcNms) {
