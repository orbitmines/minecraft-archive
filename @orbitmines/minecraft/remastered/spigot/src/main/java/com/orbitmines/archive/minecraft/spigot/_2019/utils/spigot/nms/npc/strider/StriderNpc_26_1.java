package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.strider;
...
public class StriderNpc_26_1 implements StriderNpc {

    private final NpcNms npcNms;

    public StriderNpc_26_1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomStriderNpc entity = new CustomStriderNpc(level, options, npcNms);

        entity.absSnapTo(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        level.addFreshEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftStrider) craftEntity).setRemoveWhenFarAway(false);
    ...
    private static class CustomStriderNpc extends Strider {

        private final Collection<Option> options;
        private final NpcNms npcNms;

        CustomStriderNpc(Level level) {
            this(level, new ArrayList<>(), null);
        }

        CustomStriderNpc(Level level, Collection<Option> options, NpcNms npcNms) {
            super(EntityType.STRIDER, level);
