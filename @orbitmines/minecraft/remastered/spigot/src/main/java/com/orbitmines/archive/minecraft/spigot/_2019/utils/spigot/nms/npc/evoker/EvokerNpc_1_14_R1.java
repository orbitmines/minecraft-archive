package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.evoker;
...
public class EvokerNpc_1_14_R1 implements EvokerNpc {

    private final NpcNms npcNms;

    public EvokerNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomEvokerNpc entity = new CustomEvokerNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftEvoker) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomEvokerRideable entity = new CustomEvokerRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomEvokerNpc extends EntityEvoker {

        private final Collection<Option> options;

        CustomEvokerNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomEvokerNpc(World world, Collection<Option> options) {
...
//    private static class CustomEvokerRideable extends EntityEvoker {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomEvokerRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomEvokerRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
