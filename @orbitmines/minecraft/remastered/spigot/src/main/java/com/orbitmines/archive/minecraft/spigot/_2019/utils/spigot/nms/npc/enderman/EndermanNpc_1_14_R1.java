package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.enderman;
...
public class EndermanNpc_1_14_R1 implements EndermanNpc {

    private final NpcNms npcNms;

    public EndermanNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomEndermanNpc entity = new CustomEndermanNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftEnderman) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomEndermanRideable entity = new CustomEndermanRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomEndermanNpc extends EntityEnderman {

        private final Collection<Option> options;

        CustomEndermanNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomEndermanNpc(World world, Collection<Option> options) {
...
//    private static class CustomEndermanRideable extends EntityEnderman {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomEndermanRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomEndermanRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
