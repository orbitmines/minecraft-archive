package com.orbitmines.archive.minecraft.spigot._2019.utils.spigot.nms.npc.elder_guardian;
...
public class ElderGuardianNpc_1_14_R1 implements ElderGuardianNpc {

    private final NpcNms npcNms;

    public ElderGuardianNpc_1_14_R1(NpcNms npcNms) {
        this.npcNms = npcNms;
    }
    ...
    public Entity spawn(Location location, Collection<Option> options) {
        ...
        CustomGuardianElderNpc entity = new CustomGuardianElderNpc(world, options);

        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
        world.addEntity(entity);

        CraftEntity craftEntity = entity.getBukkitEntity();
        ((CraftElderGuardian) craftEntity).setRemoveWhenFarAway(false);
...
//        World world = ((CraftWorld) location.getWorld()).getHandle();
//        CustomGuardianElderRideable entity = new CustomGuardianElderRideable(world, npcNms, speed, backMultiplier, sideMultiplier, walkHeight, jumpHeight);
//
//        entity.setPositionRotation(location.getX(), location.getY(), location.getZ(), location.getYaw(), location.getPitch());
//        world.addEntity(entity);
//
...
//    }
    ...
    private static class CustomGuardianElderNpc extends EntityGuardianElder {

        private final Collection<Option> options;

        CustomGuardianElderNpc(World world) {
            this(world, new ArrayList<>());
        }

        CustomGuardianElderNpc(World world, Collection<Option> options) {
...
//    private static class CustomGuardianElderRideable extends EntityGuardianElder {
//
//        private final NpcNms npcNms;
//
...
//
//        CustomGuardianElderRideable(World world) {
//            this(world, null, 0F, 0F, 0F, 0F, 0F); //DEFAULTS
//        }
//
//        CustomGuardianElderRideable(World world, NpcNms npcNms, float speed, float backMultiplier, float sideMultiplier, float walkHeight, float jumpHeight) {
