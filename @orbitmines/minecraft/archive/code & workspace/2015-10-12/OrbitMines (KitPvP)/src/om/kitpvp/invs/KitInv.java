...
public class KitInv extends InventoryInstance {
	...
	private ItemStack[] getContects(Player player){
		...
		contents[45] = ItemUtils.setDisplayname(new ItemStack(Material.REDSTONE_BLOCK, 1), "§4§l§o<< Back");
	...
	private ItemStack getItem(OMPlayer omp, int level){
		if(kitpvpkit.getMaxLevel() >= level){
			return ItemUtils.setDisplayname(new ItemStack(Material.NETHER_STAR, level), "§b§l" + kitpvpkit.getName() + " §7§o(§a§oLvL " + level + "§7§o)");
		}
		return ItemUtils.setItem(new ItemStack(Material.INK_SACK, level), "§4§l§oUnavailable", 1);
	}
}
