		...
		if(c instanceof ProxiedPlayer){
			...
			if(!a[0].startsWith("/")){
				...
				if(bp.hasPassword() && !bp.isLoggedIn()){
					...
					if(a[0].startsWith("/")){
						for(BungeePlayer bplayer : BungeePlayer.getPlayers()){
							if(bplayer.canShowStaffMessage()){
					...
					if(a[0].equalsIgnoreCase("/silent")){
						...
						p.sendMessage("§7Connecting to §6Hub§7...");
						...
						if(a.length == 1){
							p.sendMessage("§7Currently connected to §6" + bp.getServer().getNameWithoutColor() + "§7.");
						}
						else if(a.length == 2){
							if(ProxyServer.getInstance().getServers().containsKey(a[1])){
								...
								p.sendMessage("§7Connecting to §6" + a[1].substring(0, 1).toUpperCase() + a[1].substring(1) + "§7...");
							...
							if(!bp.onCooldown(Cooldown.REPORT)){
								...
								if(p2 != null){	
									if(p2 != p){
										...
										p.sendMessage("§7You §c§lREPORTED §c" + p2.getName() + " for §c" + reason + "§7.");
