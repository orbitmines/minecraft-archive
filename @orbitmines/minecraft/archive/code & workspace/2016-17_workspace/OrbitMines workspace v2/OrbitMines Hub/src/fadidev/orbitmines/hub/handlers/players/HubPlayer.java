...
import fadidev.orbitmines.hub.utils.IPUtils;
...
public class HubPlayer extends OMPlayer {
    ...
    public void loadPlayerData() {
        ...
        if(getLanguage() == Language.DUTCH && (country == null || (!country.equals("Netherlands") && !country.equals("Belgium")))){
            new BukkitRunnable(){
                @Override
                public void run() {
                    getPlayer().sendMessage("");
                    getPlayer().sendMessage("§c§lIMPORTANT:");
                    getPlayer().sendMessage("§7Welcome to OrbitMines, " + getColorName() + "§7!");
                    getPlayer().sendMessage("§7The main language on OrbitMines is §c§lDutch§7. It seems like you're not from the Netherlands nor from Belgium. We have added an option for players like you to change the language of all messages on our server. Click on the §c§lRedstone Torch§7, then click on the §c§lBanner§7 to switch to §c§lEnglish§7.");
                }
            }.runTaskLater(hub, 30);
        }
    ...
    private void loadTickets(String uuid){
        if(Database.get().containsPath("MiniGames-Tickets", "uuid", "uuid", uuid)){
            ...
            if(!ticketString.equals("")){
                ...
                for(String ticket : tickets){
                    ...
                    if(!ticketParts[0].equals("-1")) /* clear ChickenMama database */
        ...
        if(getTicket(TicketType.CHICKEN_MAMA_KIT) == null){
            ...
            for(TicketType type : Ticket.KITS){
                addTicketAmount(type, 1);
            }
        }
    }
