    ...
    * Below Cosmetic Perks:
    ...
    public void setWardrobeDisco(boolean wardrobeDisco){
        if(wardrobeDisco){
            ...
            getPlayer().sendMessage(Messages.ENABLE_WARDROBE_DISCO.get(this, ColorUtils.getColor(ColorUtils.random(ColorUtils.WARDROBE))));
            discoWardrobe();
        }
    ...
    public void setVIP(VIPRank vipRank){
        ...
        Title t = new Title("", Messages.RANK_RECEIVE.get(this, vipRank == VIPRank.EMERALD_VIP || vipRank == VIPRank.IRON_VIP ? "an" : "a", vipRank.getRankString() + " VIP"), 20, 40, 20);
        t.send(getPlayer());
    }
    ...
    public void discoWardrobe(){
        ...
        p.getInventory().setChestplate(ItemUtils.addColor(ItemUtils.itemstack(Material.LEATHER_CHESTPLATE, 1, c + "Disco Armor"), color));
        p.getInventory().setLeggings(ItemUtils.addColor(ItemUtils.itemstack(Material.LEATHER_LEGGINGS, 1, c + "Disco Armor"), color));
        p.getInventory().setBoots(ItemUtils.addColor(ItemUtils.itemstack(Material.LEATHER_BOOTS, 1, c + "Disco Armor"), color));
    }
    ...
    public void wardrobe(Color color){
        ...
        p.sendMessage(Messages.ENABLE_WARDROBE.get(this, ColorUtils.getName(color)));

        p.getInventory().setChestplate(ItemUtils.addColor(ItemUtils.itemstack(Material.LEATHER_CHESTPLATE, 1, c + " Armor"), color));
        p.getInventory().setLeggings(ItemUtils.addColor(ItemUtils.itemstack(Material.LEATHER_LEGGINGS, 1, c + " Armor"), color));
        p.getInventory().setBoots(ItemUtils.addColor(ItemUtils.itemstack(Material.LEATHER_BOOTS, 1, c + " Armor"), color));
    }
    ...
    public void disguiseAsMob(EntityType type, boolean baby, Player... players){
        Entity entity = api.getNms().entity().disguiseAsMob(getPlayer(), type, baby, players);
    ...
    public void sendQuitMessage(){
        if(api.getServerType() != Server.MINIGAMES){
            if(isSilent()){
                String message = Messages.QUIT_SILENT.get(this, getName());
                ...
                String message = Messages.QUIT.get(this, getName());
                for(OMPlayer omp : api.getOMPlayers()){
                    omp.getPlayer().sendMessage(message);
                }
            }
    ...
    public void sendJoinMessage(){
        ...
        if(api.getServerType() != Server.MINIGAMES){
            if(isSilent()){
                String message = Messages.JOIN_SILENT.get(this, getName());
                for(OMPlayer omp : api.getOMPlayers()){
                    if(omp.hasPerms(StaffRank.MODERATOR))
                        omp.getPlayer().sendMessage(message);
                }
                ...
                String message = Messages.JOIN.get(this, getName());
                for(OMPlayer omp : api.getOMPlayers()){
                    omp.getPlayer().sendMessage(message);
                }
            }
                        ...
                        if(!hasPlayersEnabled()){
                            ...
                            if(omp.isDisguised()){
                                if(omp.getDisguiseEntityType() != null){
                                    new BukkitRunnable(){
                                        public void run(){
                                            omp.disguiseAsMob(omp.getDisguiseEntityType(), isDisguisedBaby(), getPlayer());
                                        }
                                    }.runTaskLater(api, 40);
                                }
                                else{
    ...
    private void loadFirework(String uuid){
        ...
        if(Database.get().containsPath("Fireworks-Settings", "uuid", "uuid", uuid)){
            ...
            this.fireworkSettings = new FireworkSettings(Color.AQUA, null, null, null, false, false, FireworkEffect.Type.BALL);
        }
    }
