...
import static fadidev.orbitmines.hub.utils.enums.MiniGameType.ULTRA_HARD_CORE;
...
public class MiniGameTicketInv extends OMInventory {
    ...
    public MiniGameTicketInv(MiniGameType type, InvType invType){
        ...
        this.itemNms = OrbitMinesHub.getHub().getApi().getNms().customItem();
    }
            ...
            case SKYWARS:
                if(getInvType() == InvType.KITS){
                    for(TicketType type : Ticket.SKYWARS_KITS){
                        Utils.consoleWarning(type.toString());
                        contents[type.getSlot()] = getItemStack(omp, type);
                    }
                }
                else{
    ...
    private ItemStack getItemStack(HubPlayer omp, TicketType type){
        ...
        lore.add(" §7" + HubMessages.WORD_RARITY.get(omp) + ": " + type.getRarity().getName() + " ");
        lore.add("");
