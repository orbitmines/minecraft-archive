...
public class ShopSign {
	...
	public void buyItems(OMPlayer omp){
		...
		if(p2 != null){
			...
			p.sendMessage("§7Je hebt §6" + new SurvivalUtils().getMaterialName(m) + "§7(§6" + this.amount + "§7) gekocht van §6" + p2.getName() + " §7voor §6" + this.price + currency + "§7.");
			p2.sendMessage("§6" + p.getName() + "§7 heeft §6" + new SurvivalUtils().getMaterialName(m) + "§7(§6" + this.amount + "x§7) gekocht van jou voor §6" + this.price + currency + "§7.");
		}
		else{
			p.sendMessage("§7Je hebt §6" + new SurvivalUtils().getMaterialName(m) + "§7(§6" + this.amount + "§7) gekocht van §6" + Utils.getName(this.uuid) + " §7voor §6" + this.price + currency + "§7.");
	...
	public void sellItems(OMPlayer omp){
		...
		if(p2 != null){
			...
			omp.getPlayer().sendMessage("§7Je hebt §6" + new SurvivalUtils().getMaterialName(m) + "§7(§6" + this.amount + "§7) verkocht aan §6" + p2.getName() + " §7voor §6" + this.price + currency + "§7.");
			...
			omp.getPlayer().sendMessage("§7Je hebt §6" + new SurvivalUtils().getMaterialName(m) + "§7(§6" + this.amount + "§7) verkocht aan §6" + Utils.getName(this.uuid) + " §7aan §6" + this.price + currency + "§7.");
	...
	public void delete(){
		...
		new BukkitRunnable(){
			public void run(){
				...
				if(p != null){
					if(ServerData.isServer(Server.SURVIVAL)){
						OMPlayer.getOMPlayer(p).getSurvivalPlayer().getShopSigns().remove(sign);
					}
					else{
						OMPlayer.getOMPlayer(p).getPrisonPlayer().getShopSigns().remove(sign);
					}
				}
				getShopSigns().remove(sign);
				saveToConfig();
			}
