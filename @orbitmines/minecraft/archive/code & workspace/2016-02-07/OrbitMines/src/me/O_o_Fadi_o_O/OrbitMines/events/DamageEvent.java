							...
							if(ompE.hasStackerEnabled()){
								if(ompD.hasStackerEnabled()){
									if(ompE.hasPlayersEnabled()){
										...
										pD.sendMessage("§7Je hebt " + ompE.getName() + "§7 op je hoofd gezet!");
										pD.playEffect(pD.getLocation(), Effect.STEP_SOUND, 152);
										pE.sendMessage("§7" + ompD.getName() + " §7heeft je op zijn/haar hoofd gezet!");
										e.setCancelled(true);
								}
								else{
									pD.sendMessage("§7" + ompE.getName() + "§7heeft §3§lSpelers§7 §c§lUIT§7 staan.");
								}
							}
							else{
								pD.sendMessage("§7Je hebt §3§lSpelers§7 §c§lUIT§7 staan! Zet het aan in je §c§nInstellingen§7.");
							}
						}
						else{
							pD.sendMessage("§7" + ompE.getName() + "§7heeft §3§lSpelers§7 §c§lUIT§7 staan.");
					...
					if(arena.getSpectators().contains(ompD)){
						...
						if(arena.getState() != GameState.IN_GAME){
							...
							if(kitD != null && kitD.getKitName().equals(TicketType.RUNNER_KIT.getKit().getKitName())){
								e.setCancelled(true);
								pD.sendMessage("§7Je kan geen schade oplopen in de eerste 30s! (§f§l" + TicketType.RUNNER_KIT.getName() + "§7)");
							}
								...
								if(kitE != null && kitE.getKitName().equals(TicketType.RUNNER_KIT.getKit().getKitName())){
									e.setCancelled(true);
									pD.sendMessage("§7Je kan " + ompE.getName() + "§7 niet aanvallen in de eerste 30s! (§f§l" + TicketType.RUNNER_KIT.getName() + "§7)");
								}
							}
