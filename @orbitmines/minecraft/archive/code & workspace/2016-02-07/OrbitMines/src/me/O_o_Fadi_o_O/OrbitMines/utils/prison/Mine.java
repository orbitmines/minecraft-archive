				...
				if(pp.hasPerms(getRank())){
					...
					if(p.getLocation().distance(getTimerSign()) <= 16){
						...
						if(pp.isClockEnabled()){
							clocksign[0] = "Lagg Problemen?";
						}
						
						clocksign[1] = "§lToggle Clock";
						clocksign[2] = "";
						
						clocksign[3] = "§lClock §c§lUIT";
						if(pp.isClockEnabled()){
							clocksign[3] = "§lClock §a§lAAN";
						}
	...
	public void reset(OMPlayer omp){
		...
		for(Player p : Bukkit.getOnlinePlayers()){
			if(isInMine(p.getLocation())){
				...
				if(omp != null){
					p.sendMessage("§7Mijn §a§l" + getRank().toString() + "§7 is gereset door " + omp.getName() + "§7!");
				}
				else{
					p.sendMessage("§7Mijn §a§l" + getRank().toString() + "§7 is gereset!");
				}
			}
		}
