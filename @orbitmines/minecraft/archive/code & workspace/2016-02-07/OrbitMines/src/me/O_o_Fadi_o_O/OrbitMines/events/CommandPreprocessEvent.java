		...
		if(omp.isLoaded()){
			if(ServerData.isServer(Server.CREATIVE) && omp.getCreativePlayer().isInPvPPlot() && !omp.isOpMode()){
				...
				if(a[0].equalsIgnoreCase("/confirm")){
					...
					p.sendMessage("§7Je kan geen commands uitvoeren terwijl je de §2PvP Area§7 aan het joinen bent.");
				}
			}
				...
				if(a[0].equalsIgnoreCase("/spawn")){
					...
					p.sendMessage("§7Je kan geen commands uitvoeren terwijl je in de §2PvP Area§7 bent. (Verlaat de arena met §6/spawn§7)");
				}
			}
				...
				if(!found){
					...
					if(topic == null && !Command.isCommand(a[0]) || a[0].toLowerCase().equals("/me")){
						if(!a[0].toLowerCase().startsWith("/bukkit:")){
							...
							e.setCancelled(true);
							p.sendMessage("§7Je hebt geen toegang om §6" + a[0].toLowerCase() + "§7 te gebruiken!");
						}
					}
				}
			}
