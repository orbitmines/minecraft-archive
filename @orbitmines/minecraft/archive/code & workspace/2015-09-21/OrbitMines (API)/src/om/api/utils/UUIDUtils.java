...
public class UUIDUtils {
	...
	private Map<String, UUID> playerUUIDs;
	private Map<UUID, String> playerNames;
	
	public UUIDUtils(){
		playerUUIDs = new HashMap<String, UUID>();
		playerNames = new HashMap<UUID, String>();
	}
	...
	public String getNameDate(UUID uuid){
		NameFetcher namef = new NameFetcher(uuid);
		try{
			String name = namef.call().get(uuid).get(namef.call().get(uuid).size() -1);
	...
	public HashMap<String, String> getNames(UUID uuid){
		NameFetcher namef = new NameFetcher(uuid);
		try{
	...
	public Map<String, UUID> getPlayerUUIDs() {
		return playerUUIDs;
	}
}
