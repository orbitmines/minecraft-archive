...
public class NPC {
	...
	public void moveToLocation(Location location){
		try{
			...
			Object entityLiving = api.getReflectionUtils().getMethod("getHandle", le.getClass(), 0).invoke(le);
			Object nav = api.getReflectionUtils().getMethod("getNavigation", entityLiving.getClass(), 0).invoke(entityLiving);
			api.getReflectionUtils().getMethod("a", nav.getClass(), 4).invoke(nav, location.getX(), location.getY(), location.getZ(), 1.2);
		}catch(Exception ex){
			ex.printStackTrace();
		}
	}
