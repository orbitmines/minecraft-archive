...
public class Plot {
	...
	public void setPvPSpawns(List<Location> pvpspawns) {
		...
		for(Location pvpspawn : this.pvpspawns){
			ConfigManager.plots.set("plots." + getPlotID() + ".PvPInfo.Locations.Spawns." + index, Utils.getStringFromLocation(pvpspawn));
	...
	public void setPvPKits(List<Kit> pvpkits) {
		...
		for(Kit kit : this.pvpkits){
			kit.saveToConfig(ConfigManager.plots, "plots." + getPlotID() + ".PvPInfo.Kits." + kit.getKitName());
		}
		ConfigManager.savePlots();
	}
	...
	public void updateNPC(Kit kit){
		...
		npcas.setCustomName("§7§lKit §8| §a" + kit.getKitName());
		npcas.setCustomNameVisible(true);
		npcas.setHelmet(kit.getHelmet());
		npcas.setChestplate(kit.getChestplate());
		npcas.setLeggings(kit.getLeggings());
		npcas.setBoots(kit.getBoots());
		npcas.setItemInHand(kit.getFirstItem());
		npcas.update();
	}
