...
public class Start extends Plugin {
		...
		new BungeeScheduler(){}.schedule(this, new Runnable(){
			public void run(){
				for(ProxiedPlayer p : ProxyServer.getInstance().getPlayers()){
					try{
						connectedto.put(getUUIDfromPlayer(p), p.getServer().getInfo());
					}catch(Exception ex){
						connectedto.put(getUUIDfromPlayer(p), ProxyServer.getInstance().getServerInfo("hub"));
					}
				}
			}
	...
	public static UUID getUUIDfromPlayer(ProxiedPlayer p){
