			...
			if(Start.staffpasswords.containsKey(p.getName()) && Start.stafflogin.get(p) == false){
				...
				if(lowercaseMessage.startsWith("/") && !lowercaseMessage.startsWith("/l") && !lowercaseMessage.startsWith("/login")){
					for(ProxiedPlayer player : ProxyServer.getInstance().getPlayers()){
						if(Start.hasStaffPerms(player, StaffRank.Moderator)){
							if(Start.showstaffmessage.get(Start.getUUIDfromPlayer(player)) == true){
					...
					if(Start.silent.containsKey(uuid)){
						if(a[0].equalsIgnoreCase("/silent")){
							...
							if(Start.silent.get(uuid) == true){
								...
								DatabaseManager.setSilentJoin(p.getName(), false);
							}
							else{
								p.sendMessage("§6§lSilent Mode §8| §a§lENABLED§7!");
								Start.silent.put(uuid, true);
								DatabaseManager.setSilentJoin(p.getName(), true);
							}
						}
					}
				}
				...
				if(lowercaseMessage.startsWith("/list") || lowercaseMessage.startsWith("/glist")){
					...
					if(a[0].equalsIgnoreCase("/list") || a[0].equalsIgnoreCase("/glist")){
						...
						try{for(ProxiedPlayer player : kitpvp.getPlayers()){if(!Start.silent.containsKey(Start.getUUIDfromPlayer(player)) || Start.silent.containsKey(Start.getUUIDfromPlayer(player)) && Start.silent.get(Start.getUUIDfromPlayer(player)) == false){if(kitpvpplayers.equals("")){kitpvpplayers = "§f" + player.getName();}else{kitpvpplayers = kitpvpplayers + "§8, §f" + player.getName();}}else{kitpvpsilent = kitpvpsilent +1;}}}catch(Exception ex){kitpvponline = false;}
						try{for(ProxiedPlayer player : prison.getPlayers()){if(!Start.silent.containsKey(Start.getUUIDfromPlayer(player)) || Start.silent.containsKey(Start.getUUIDfromPlayer(player)) && Start.silent.get(Start.getUUIDfromPlayer(player)) == false){if(prisonplayers.equals("")){prisonplayers = "§f" + player.getName();}else{prisonplayers = prisonplayers + "§8, §f" + player.getName();}}else{prisonsilent = prisonsilent +1;}}}catch(Exception ex){prisononline = false;}
						try{for(ProxiedPlayer player : creative.getPlayers()){if(!Start.silent.containsKey(Start.getUUIDfromPlayer(player)) || Start.silent.containsKey(Start.getUUIDfromPlayer(player)) && Start.silent.get(Start.getUUIDfromPlayer(player)) == false){if(creativeplayers.equals("")){creativeplayers = "§f" + player.getName();}else{creativeplayers = creativeplayers + "§8, §f" + player.getName();}}else{creativesilent = creativesilent +1;}}}catch(Exception ex){creativeonline = false;}
						try{for(ProxiedPlayer player : hub.getPlayers()){if(!Start.silent.containsKey(Start.getUUIDfromPlayer(player)) || Start.silent.containsKey(Start.getUUIDfromPlayer(player)) && Start.silent.get(Start.getUUIDfromPlayer(player)) == false){if(hubplayers.equals("")){hubplayers = "§f" + player.getName();}else{hubplayers = hubplayers + "§8, §f" + player.getName();}}else{hubsilent = hubsilent +1;}}}catch(Exception ex){hubonline = false;}
						try{for(ProxiedPlayer player : survival.getPlayers()){if(!Start.silent.containsKey(Start.getUUIDfromPlayer(player)) || Start.silent.containsKey(Start.getUUIDfromPlayer(player)) && Start.silent.get(Start.getUUIDfromPlayer(player)) == false){if(survivalplayers.equals("")){survivalplayers = "§f" + player.getName();}else{survivalplayers = survivalplayers + "§8, §f" + player.getName();}}else{survivalsilent = survivalsilent +1;}}}catch(Exception ex){survivalonline = false;}
						try{for(ProxiedPlayer player : skyblock.getPlayers()){if(!Start.silent.containsKey(Start.getUUIDfromPlayer(player)) || Start.silent.containsKey(Start.getUUIDfromPlayer(player)) && Start.silent.get(Start.getUUIDfromPlayer(player)) == false){if(skyblockplayers.equals("")){skyblockplayers = "§f" + player.getName();}else{skyblockplayers = skyblockplayers + "§8, §f" + player.getName();}}else{skyblocksilent = skyblocksilent +1;}}}catch(Exception ex){skyblockonline = false;}
						try{for(ProxiedPlayer player : minigames.getPlayers()){if(!Start.silent.containsKey(Start.getUUIDfromPlayer(player)) || Start.silent.containsKey(Start.getUUIDfromPlayer(player)) && Start.silent.get(Start.getUUIDfromPlayer(player)) == false){if(minigamesplayers.equals("")){minigamesplayers = "§f" + player.getName();}else{minigamesplayers = minigamesplayers + "§8, §f" + player.getName();}}else{minigamessilent = minigamessilent +1;}}}catch(Exception ex){minigamesonline = false;}
						try{for(ProxiedPlayer player : alpha.getPlayers()){if(!Start.silent.containsKey(Start.getUUIDfromPlayer(player)) || Start.silent.containsKey(Start.getUUIDfromPlayer(player)) && Start.silent.get(Start.getUUIDfromPlayer(player)) == false){if(alphaplayers.equals("")){alphaplayers = "§f" + player.getName();}else{alphaplayers = alphaplayers + "§8, §f" + player.getName();}}else{alphasilent = alphasilent +1;}}}catch(Exception ex){alphaonline = false;}
					...
					if(a[0].equalsIgnoreCase("/friends")){
						...
						if(a.length == 1){
							...
							if(a[1].equalsIgnoreCase("remove")){
								UUID friendUUID = Start.getUUIDfromString(a[2]);
								if(FriendManager.playerfriends.containsKey(uuid) && FriendManager.playerfriends.get(uuid).contains(friendUUID)){
									...
									if(friends.size() != 0){
										...
										DatabaseManager.setFriends(p.getName(), friendsS);
									}
									else{
										FriendManager.playerfriends.remove(uuid);
										DatabaseManager.removeFriends(p.getName());
									}
									...
									if(!FriendManager.playerfriends.containsKey(friendUUID)){
										String[] friends2 = DatabaseManager.getString(a[2], "Friends", "friends").split("\\|");
									...
									if(friends2.size() != 0){
										...
										DatabaseManager.setFriends(a[2], friendsS2);
									}
									else{
										FriendManager.playerfriends.remove(friendUUID);
										DatabaseManager.removeFriends(a[2]);
									}
									...
									if(FriendManager.playerfriends.containsKey(uuid)){
										if(!FriendManager.playerfriends.get(uuid).contains(Start.playerfriendrequest.get(uuid))){
											if(FriendManager.playerfriends.get(uuid).size() != FriendManager.getAmountFriendsAllowed(p)){
												...
												String friend = Start.getPlayerNamefromUUID(friendUUID);
												...
												if(FriendManager.playerfriends.containsKey(friendUUID)){
													...
													DatabaseManager.setFriends(friend, friendsS2);
													...
													DatabaseManager.insertString(friend, "Friends", "friends", uuid.toString());
										...
										DatabaseManager.insertString(p.getName(), "Friends", "friends", friendUUID.toString());
										p.sendMessage("§2§lFriends §8| §7You're now friends with §2" + friend + "§7!");
					...
					if(a[0].equalsIgnoreCase("/friend")){
						...
						if(a.length == 1){
							...
							if(invitedPlayer != null){	
								if(invitedPlayer != p){
									if(!FriendManager.playerfriends.containsKey(Start.getUUIDfromPlayer(p))){
										...
										Start.playerfriendrequest.put(Start.getUUIDfromPlayer(invitedPlayer), uuid);
									}
									else{
										try{
											if(!FriendManager.playerfriends.get(Start.getUUIDfromPlayer(p)).contains(Start.getUUIDfromString(a[1]))){
												if(FriendManager.playerfriends.get(Start.getUUIDfromPlayer(p)).size() != FriendManager.getAmountFriendsAllowed(p)){
													invitedPlayer.sendMessage("§2§lFriends §8| §2" + p.getName() + "§7 wants to be friends.");
													TextComponent accept = new TextComponent("§a§lAccept");
													accept.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/friends accept"));
													accept.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder("§7Click Here to §aAccept§7.").create()));
				...
				if(lowercaseMessage.startsWith("/ip")){
					...
					if(a[0].equalsIgnoreCase("/ip")){
						...
						if(Start.hasStaffPerms(p, StaffRank.Owner)){
							if(a.length == 2){
								String player = a[1];
								UUID playerUUID = Start.getUUIDfromString(player);
								...
								if(Start.ips.containsKey(playerUUID)){
									...
									try{
										if(DatabaseManager.containsPath("uuid", "PlayerIPs", playerUUID.toString())){
											Start.ips.put(playerUUID, DatabaseManager.getString(player, "PlayerIPs", "ip"));
											p.sendMessage("§4§lOP §8| §7" + player + "'s IP is §6" + Start.ips.get(playerUUID) + "§7.");
										}
										else{
					...
					if(a[0].equalsIgnoreCase("/unban")){
						...
						if(Start.hasStaffPerms(p, StaffRank.Moderator)){
							...
							if(a.length == 2){
								...
								try{
									if(DatabaseManager.containsPath("uuid", "PlayerBans", Start.getUUIDfromString(a[1]).toString())){
					...
					if(a[0].equalsIgnoreCase("/ban")){
						...
						if(Start.hasStaffPerms(p, StaffRank.Moderator)){
							...
							if(a.length >= 7){
								...
								try{
									if(!DatabaseManager.containsPath("uuid", "PlayerBans", Start.getUUIDfromString(BannedPlayer).toString())){
										...
										try{
											...
											for(ProxiedPlayer player : ProxyServer.getInstance().getPlayers()){
												...
												if(player.getName().equalsIgnoreCase(BannedPlayer)){
											        UUID banneduuid = Start.getUUIDfromString(BannedPlayer);
				...
				if(lowercaseMessage.startsWith("/msg") || lowercaseMessage.startsWith("/m") || lowercaseMessage.startsWith("/t") || lowercaseMessage.startsWith("/tell") || lowercaseMessage.startsWith("/w") || lowercaseMessage.startsWith("/whisper")){
					...
					if(a[0].equalsIgnoreCase("/msg") || a[0].equalsIgnoreCase("/m") || a[0].equalsIgnoreCase("/t") || a[0].equalsIgnoreCase("/tell") || a[0].equalsIgnoreCase("/w") || a[0].equalsIgnoreCase("/whisper")){
						...
						if(a.length >= 3){
							...
							if(toPlayer != null){
								UUID toUUID = Start.getUUIDfromPlayer(toPlayer);
					...
					if(a[0].equalsIgnoreCase("/r") || a[0].equalsIgnoreCase("/reply")){
						...
						if(a.length >= 2){
							if(Start.lastmsg.containsKey(uuid)){
								...
								for(ProxiedPlayer player : ProxyServer.getInstance().getPlayers()){
									if(Start.lastmsg.get(uuid) == Start.getUUIDfromPlayer(player)){
										toPlayer = player;
									}
								}
