		...
		if(StorageManager.useantiadvertise == true){
		    		...
		    		if(!StorageManager.whitelistedips.contains(sb.toString().replace(" ", "."))){
			    		...
			    		if(m.find() == true){
			    			if(StorageManager.kick == true){
			    				...
			    				Start.sendMessageNullCheck(p, TitleManager.importTitle(p, StorageManager.advertisemessage.replace("&", "§").replace("%type%", StorageManager.typevariableips.replace("&", "§"))));
	    	...
	    	if(StorageManager.canceldomainnames == true && canchat){
				if(!p.hasPermission("BungeeMSG.bypass.domainnames") && !p.hasPermission("BungeeMSG.bypass.*")){
		    		...
		    		if(lowercasemsg.contains("www.") || lowercasemsg.contains("wwwdot") || lowercasemsg.contains("www dot") || lowercasemsg.contains(".net") || lowercasemsg.contains(".com") || lowercasemsg.contains(".co.uk") || lowercasemsg.contains("http://") || lowercasemsg.contains("https://") || lowercasemsg.contains(".de") || lowercasemsg.contains(".nl") || lowercasemsg.contains(".be") || lowercasemsg.contains(".fr") || lowercasemsg.contains(".org") || lowercasemsg.contains(".us")){
		    			if(StorageManager.kick == true){
		    				...
		    				Start.sendMessageNullCheck(p, TitleManager.importTitle(p, StorageManager.advertisemessage.replace("&", "§").replace("%type%", StorageManager.typevariabledomainnames.replace("&", "§"))));
