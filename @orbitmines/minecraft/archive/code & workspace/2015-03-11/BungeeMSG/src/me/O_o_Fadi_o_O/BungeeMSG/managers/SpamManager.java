				...
				if(!p.hasPermission("BungeeMSG.bypass.cooldown") && !p.hasPermission("BungeeMSG.bypass.*")){
					...
					if(pm == true){
						...
						if(System.currentTimeMillis()-lastchat >= lastchatmillis){
							...
							if(timeleft == 1){
								Start.sendMessageNullCheck(p, TitleManager.importTitle(p, StorageManager.spamcooldown.replaceAll("&", "§").replaceAll("%left%", "" + timeleft).replaceAll("%second-grammer%", StorageManager.secondgrammervariableone.replaceAll("&", "§"))));
							}
							else{
								Start.sendMessageNullCheck(p, TitleManager.importTitle(p, StorageManager.spamcooldown.replaceAll("&", "§").replaceAll("%left%", "" + timeleft).replaceAll("%second-grammer%", StorageManager.secondgrammervariablemultiple.replaceAll("&", "§"))));
							}
					...
					if(StorageManager.lastmessage.containsKey(p)){
						...
						if(StorageManager.duplicatesensitivity != 0){
							if(message.toLowerCase().startsWith(lastmessage.toLowerCase())){
								canchat = false;
								Start.sendMessageNullCheck(p, TitleManager.importTitle(p, StorageManager.spamduplicate.replaceAll("&", "§")));
					...
					if(StorageManager.tofaststartedcount.containsKey(p) && StorageManager.tofastamount.containsKey(p)){
						...
						if(System.currentTimeMillis()-lastchat >= lastchatmillis){
							...
							if(StorageManager.tofastamount.get(p) >= StorageManager.tofastmaxusage){
								canchat = false;
								Start.sendMessageNullCheck(p, TitleManager.importTitle(p, StorageManager.spamtofast.replaceAll("&", "§")));
