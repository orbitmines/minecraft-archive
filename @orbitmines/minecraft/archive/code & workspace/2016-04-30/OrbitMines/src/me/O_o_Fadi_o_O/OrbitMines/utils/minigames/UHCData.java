...
public class UHCData {
	...
	public void pvp(){
		...
		arena.sendMessage(" §f§lUHC §7- §cPvP §a§lAAN");
		arena.sendMessage("");
		arena.sendMessage(" §a§oDe Border zal §l1.5 Blokken§a§o krimpen per seconde.");
		arena.sendMessage(" §a§oDe wereld zal §l100x100 Blokken§a§o zijn na 10 minuten.");
		arena.sendMessage("§6§m--------------------------------------------------");
	}
	...
	public void ending(){
		...
		arena.playSound(Sound.WITHER_DEATH, 5, 1);
		arena.sendMessage("§6§m--------------------------------------------------");
		arena.sendMessage(" §f§lUHC §7- §6Resultaten");
		arena.sendMessage("");
	...
	public void start(){
		...
		arena.playSound(Sound.WITHER_DEATH, 5, 1);
		arena.sendMessage("§cPvP§7 aan in §6" + (arena.getMinutes() -15) + "m " + arena.getSeconds() + "s§7.");
	...
	public void warmup(){
		...
		arena.sendMessage("");
		arena.sendMessage(" §a§oVerzamel llot.");
		arena.sendMessage(" §a§oPvP aan na 15 minuten.");
		arena.sendMessage(" §a§oDe border zal 1.5 Blokken per seconde krimpen na 15 minuten.");
		arena.sendMessage(" §a§oDe laatste speler levend wint!");
		arena.sendMessage(" §a§oDe Nether staat uit.");
		arena.sendMessage("");	
		arena.sendMessage(" §7Spawn Gebouwd Door: §6" +  arena.getMap().getBuilder());
		arena.sendMessage("§6§m--------------------------------------------------");
		arena.sendMessage("§7Alle Spelers Teleporten...");
	...
	public void rewardPlayers(){
		for(OMPlayer omp : arena.getAllPlayers()){
			...
			if(arena.isPlayer(omp) || arena.isSpectator(omp) && arena.getDeadPlayers().contains(omp)){
				...
				p.sendMessage("§6§m--------------------------------------------------");
				p.sendMessage(" §f§lUHC §7- §6Beloningen");
				p.sendMessage("");
				if(getFirstPlace() == omp){
					p.sendMessage(" §f§l+200 §7(1e Plaats)");
					amount += 200;
				}
				else if(getSecondPlace() == omp){
					p.sendMessage(" §f§l+120 §7(2e Plaats)");
					amount += 120;
				}
				else if(getThirdPlace() == omp){
					p.sendMessage(" §f§l+80 §7(3e Plaats)");
					amount += 80;
				}
				else{
					p.sendMessage(" §f§l+40 §7(Geduld)");
					amount += 40;
				}
