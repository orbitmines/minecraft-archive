...
public class ChickenFightData {
	...
	public void ending(){
		...
		arena.sendMessage(" §f§lChicken Fight §7- §6Resultaten");
		arena.sendMessage("");
	...
	public void warmup(){
		...
		arena.sendMessage("");
		arena.sendMessage(" §a§oDood je tegenstanders en blijf als laatst over.");
		arena.sendMessage("");
	...
	public void rewardPlayers(){
		for(OMPlayer omp : arena.getAllPlayers()){
			...
			if(arena.isPlayer(omp) || arena.isSpectator(omp) && arena.getDeadPlayers().contains(omp)){
				...
				p.sendMessage("§6§m--------------------------------------------------");
				p.sendMessage(" §f§lChicken Fight §7- §6Beloningen");
				p.sendMessage("");
				if(getFirstPlace() == omp){
					p.sendMessage(" §f§l+10 §7(1e Plaats)");
					amount += 15;
				}
				else if(getSecondPlace() == omp){
					p.sendMessage(" §f§l+5 §7(2e Plaats)");
					amount += 8;
				}
				else if(getThirdPlace() == omp){
					p.sendMessage(" §f§l+3 §7(3e Plaats)");
					amount += 4;
				}
				else{
