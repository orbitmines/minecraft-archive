	...
	private void resetLayer(final int y){
		...
		new BukkitRunnable(){
			public void run(){
				...
				if(y != 255){
					if(p != null){
						Title t = new Title("§dPlot§7 resetten...", "§d" + (y +1) + "§7/§d255", 0, 20, 0);
						t.send(p);
					}
					resetLayer(y +1);
				}
					...
					if(p != null){
						Title t = new Title("", "§7Jouw §dPlot§7 is gereset!", 0, 30, 40);
						t.send(p);
					}
				}
			}
		...
		if(plots.contains("plots." + getPlotID() + ".PvPInfo")){
			...
			if(plots.contains("plots." + getPlotID() + ".PvPInfo.Locations.NPCs")){
				...
				for(String kitname : plots.getConfigurationSection("plots." + getPlotID() + ".PvPInfo.Locations.NPCs").getKeys(false)){
					pvpnpclocations.put(getKit(kitname), Utils.getLocationFromString(plots.getString("plots." + getPlotID() + ".PvPInfo.Locations.NPCs." + kitname)));
				}
				this.pvpnpclocations = pvpnpclocations;
			}
