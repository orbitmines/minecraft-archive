...
public class SurvivalGamesData {
	...
	public void restockChests(){
		arena.sendMessage("§a§l§oAlle Chesten zijn opnieuw gevuld!");
	...
	public void ending(){
		...
		arena.sendMessage(" §f§lSurvival Games §7- §6Resultaten");
		arena.sendMessage("");
	...
	public void warmup(){
		...
		arena.sendMessage("");
		arena.sendMessage(" §a§oVerzamel loot van chests en schakel andere spelers uit.");
		arena.sendMessage("");
	...
	public void rewardPlayers(){
		for(OMPlayer omp : arena.getAllPlayers()){
			...
			if(arena.isPlayer(omp) || arena.isSpectator(omp) && arena.getDeadPlayers().contains(omp)){
				...
				p.sendMessage("§6§m--------------------------------------------------");
				p.sendMessage(" §f§lSurvival Games §7- §6Beloning");
				p.sendMessage("");
				if(getFirstPlace() == omp){
					p.sendMessage(" §f§l+25 §7(1e Plaats)");
					amount += 25;
				}
				else if(getSecondPlace() == omp){
					p.sendMessage(" §f§l+15 §7(2e Plaats)");
					amount += 15;
				}
				else if(getThirdPlace() == omp){
					p.sendMessage(" §f§l+10 §7(3e Plaats)");
					amount += 10;
				}
				else{
