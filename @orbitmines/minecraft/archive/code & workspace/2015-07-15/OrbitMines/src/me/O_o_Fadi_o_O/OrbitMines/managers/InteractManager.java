...
public class InteractManager {
	...
	public void handleSGChests(){
		Arena arena = omp.getArena();
		
		if(arena != null && arena.getType() == MiniGameType.SURVIVAL_GAMES && arena.isPlayer(omp) && b != null && (b.getType() == Material.CHEST || b.getType() == Material.TRAPPED_CHEST)){
			Chest c = (Chest) b.getState();
			
			if(!arena.getSG().getLootedChests().contains(c)){
				arena.getSG().randomChest(c);
	...
	public void handleChestShops(){
		if(b != null && (b.getType() == Material.SIGN_POST || b.getType() == Material.WALL_SIGN)){
			...
			if(sign != null){
				if(!sp.getShopSigns().contains(sign)){
	...
	public void handleSpawnInteract(){
		if(!omp.isOpMode() && p.getWorld().getName().equals(ServerData.getLobbyWorld().getName()) && e.getClickedBlock() != null){
			...
			if(m == Material.TRAP_DOOR || m == Material.CHEST || m == Material.TRAPPED_CHEST || m == Material.WORKBENCH || m == Material.FURNACE || m == Material.ENDER_CHEST || m == Material.POWERED_RAIL || m == Material.DETECTOR_RAIL || m == Material.ACTIVATOR_RAIL || m == Material.RAILS){
				e.setCancelled(true);
				omp.updateInventory();
			}
		}
