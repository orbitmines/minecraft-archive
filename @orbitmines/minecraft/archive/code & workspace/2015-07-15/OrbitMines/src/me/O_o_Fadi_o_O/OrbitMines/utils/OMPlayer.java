...
public class OMPlayer {
	...
	public void setCursive(boolean chatcolorcursive){
		...
		getPlayer().playSound(getPlayer().getLocation(), Sound.PISTON_EXTEND, 5, 1);
		getPlayer().sendMessage("" + Utils.statusString(chatcolorbold) + " §7your §" + getChatColor().getColor() +"§oCursive ChatColor§7.");
	}
	...
	public void load(){
		...
		try{
			...
			if(ServerData.isServer(Server.HUB)){
				...
				int money = 0, extrahomes = 0, extrashops = 0, extrawarps = 0;
			...
			
			{
				...
				this.swplayer = SkywarsPlayer.addSWPlayer(this.player, kills, wins, loses, beststreak);
			}
			
			//-> Ook in ChickenFightPlayer.java
			...
			if(ServerData.isServer(Server.SURVIVAL) && tp == true){
				new BukkitRunnable(){
					public void run(){
			...
			else if(ServerData.isServer(Server.HUB) && newplayer == true){
