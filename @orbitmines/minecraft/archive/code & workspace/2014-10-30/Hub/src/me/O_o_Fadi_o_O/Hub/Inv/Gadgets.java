...
public class Gadgets {
	...
	public static Inventory getGadgetsInv(Player p){
		...
		}
		{
			...
			List<String> itemlore = new ArrayList<String>();
			itemlore.add("");
			if(!Hub.gadgetsPaintballs.containsKey(p.getName())){
		...
		}
		{
			...
			List<String> itemlore = new ArrayList<String>();
			itemlore.add("");
			if(!Hub.gadgetsCreeperLauncher.containsKey(p.getName())){
		...
		}
		{
			...
			List<String> itemlore = new ArrayList<String>();
			itemlore.add("");
			if(!Hub.mushroomcow.containsKey(p.getName()) && !Hub.pig.containsKey(p.getName()) && !Hub.wolf.containsKey(p.getName())&& !Hub.sheep.containsKey(p.getName()) && !Hub.horse.containsKey(p.getName()) && !Hub.magmacube.containsKey(p.getName()) && !Hub.slime.containsKey(p.getName()) && !Hub.cow.containsKey(p.getName()) && !Hub.silverfish.containsKey(p.getName()) && !Hub.ocelot.containsKey(p.getName())){
				itemlore.add("§c§oPrice: §b100 VIP Points");
				itemlore.add("§c§oPet Required");
			}
			else{
				if(!Hub.gadgetsPetRide.containsKey(p.getName())){
					itemlore.add("§c§oPrice: §b100 VIP Points");
					itemlore.add("§7§o§nClick Here to Buy");
				}
				else{
		...
		}
		{
			...
			List<String> itemlore = new ArrayList<String>();
			itemlore.add("");
			if(!Hub.gadgetsBookExplosion.containsKey(p.getName())){
