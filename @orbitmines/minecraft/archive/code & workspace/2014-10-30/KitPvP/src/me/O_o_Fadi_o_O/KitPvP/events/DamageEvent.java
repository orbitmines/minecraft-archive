...
public class DamageEvent implements Listener{
	...
	public void onDamage(EntityDamageByEntityEvent e){
		...
		if(e.getDamager() instanceof Projectile){
			...
			if(Start.arrow.contains(proj)){
				Location l = proj.getLocation();
				World w = proj.getWorld();
				w.strikeLightning(l);
				Start.arrow.remove(proj);
			}
			if(Start.necromancerarrowlvl1.contains(proj)){
				Location l = proj.getLocation();
				...
				Start.necromancerarrowlvl1.remove(proj);
			}
			if(Start.necromancerarrowlvl2.contains(proj)){
				Location l = proj.getLocation();
				...
				Start.necromancerarrowlvl2.remove(proj);
			}
			if(Start.tntlvl2.contains(proj)){
				
				TNTPrimed tnt = (TNTPrimed) proj.getWorld().spawn(proj.getLocation(), TNTPrimed.class);
				tnt.setFuseTicks(10);
					
					
				Start.tntlvl2.remove(proj);
			}
	    }
