					...
					for(ServerInfo info : StorageManager.autoannounceservers.get(announcer)){
						for(ProxiedPlayer player : info.getPlayers()){
							for(String message : messages){
								player.sendMessage(message.replace("&", "§").replace("%receiver%", player.getName()).replace("%server-receiver%", StorageManager.servernames.get(player.getServer().getInfo()).replace("&", "§")));
							}
						}
					}
