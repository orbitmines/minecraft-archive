...
public class GhostAttackData {
	...
	public void ending(){
		...
		arena.sendMessage(" §f§lGhost Attack §7- §6Results");
		arena.sendMessage("");
		if(getGhostKiller() != null || getGhost() == null){
			if(getGhost() != null){
				arena.sendMessage(" §7§lThe Ghost has been killed by " + getGhostKiller().getName() + "§7§l!");
				arena.sendMessage("");
			}
			...
			if(arena.getPlayers().size() > 1){
				...
				arena.sendMessage(" §6Winners: " + getGhost().getName());
			}
			else{
				arena.sendMessage(" §6Winner: " + arena.getPlayers().get(0).getName());
			}
		}
		else{
			arena.sendMessage(" §7§lThe Ghost elimated all players!");
			arena.sendMessage("");
			arena.sendMessage(" §6Winner: " + getGhost().getName());
		}
		arena.sendMessage("");
		arena.sendMessage("§6§m--------------------------------------------------");
	...
	public void start(){
		...
		arena.sendMessage(ompG.getColorName() + "§7 is the §7§lGhost§7!");
	...
	public void warmup(){
		...
		arena.sendMessage("");
		arena.sendMessage(" §a§oGhost: Kill all players.");
		arena.sendMessage(" §a§oPlayers: Kill the Ghost, you'll be able to see it every once in a while.");
		arena.sendMessage("");
	...
	public void rewardPlayers(){
		for(OMPlayer omp : arena.getAllPlayers()){
			...
			if(arena.isPlayer(omp) || arena.isSpectator(omp) && arena.getDeadPlayers().contains(omp)){
				...
				p.sendMessage("§6§m--------------------------------------------------");
				p.sendMessage(" §f§lGhost Attack §7- §6Rewards");
				p.sendMessage("");
				if(isGhost(omp) && getGhostKiller() == null){
					p.sendMessage(" §f§l+20 §7(Won as the Ghost)");
					amount += 20;
				}
				else if(getGhostKiller() == omp){
					p.sendMessage(" §f§l+15 §7(Killed the Ghost)");
					amount += 15;
				}
				else{
