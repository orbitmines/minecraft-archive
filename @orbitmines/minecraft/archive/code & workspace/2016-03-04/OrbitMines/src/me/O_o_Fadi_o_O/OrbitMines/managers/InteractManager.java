...
public class InteractManager {
		...
		if(omp.hasPerms(StaffRank.Owner) && item.getType() == CageBuilder.getWand().getType() && item.getItemMeta().getDisplayName().equals(CageBuilder.getWand().getItemMeta().getDisplayName())){
			...
			if(a == Action.LEFT_CLICK_BLOCK){
				if(omp.getCageBuilder() != null){
					...
					p.sendMessage("§eLocation 1§7 neergezet: §e" + b.getLocation().getBlockX() + "§7, §e" + b.getLocation().getBlockY() + "§7, §e" + b.getLocation().getBlockZ() + "§7.");
				}
				else{
					p.sendMessage("§7Je moet eerst een nieuwe Cage Builder maken!");
				}
			}
			else if(a == Action.RIGHT_CLICK_BLOCK){
				if(omp.getCageBuilder() != null){
					omp.getCageBuilder().setL2(b.getLocation());
					p.sendMessage("§eLocation 2§7 neergezet: §e" + b.getLocation().getBlockX() + "§7, §e" + b.getLocation().getBlockY() + "§7, §e" + b.getLocation().getBlockZ() + "§7.");
				}
				else{
					p.sendMessage("§7Je moet eerst een nieuwe Cage Builder maken!");
				}
				...
				if(pp.getShop() == null){
					if(shop.canRent()){
						if(!omp.onCooldown(Cooldown.MESSAGE)){
							if(pp.hasGold(5000)){
								p.sendMessage("§7Deze shop is twee dagen van jou.");
					...
					if(shop.getShopID() == pp.getShop().getShopID()){
						if(!omp.onCooldown(Cooldown.MESSAGE)){
							if(pp.hasGold(5000)){
								p.sendMessage("§7Je hebt twee dagen bij jouw shop toegevoegd.");
	...
	public void handleFeatherAttack(){
		if(item.getType() == Material.FEATHER && item.getItemMeta().getDisplayName().equals("§f§lFeather Attack")){
			...
			if(!omp.onCooldown(Cooldown.FEATHER_ATTACK)){
				p.sendMessage("§7Je hebt §f§lFeather Attack§7 gebruikt!");
	...
	public void handleEggBomb(){
		if(item.getType() == Material.EGG && item.getItemMeta().getDisplayName().equals("§f§lEgg Bomb")){
			...
			if(!omp.onCooldown(Cooldown.EGG_BOMB)){
				p.sendMessage("§7Je hebt §f§lEgg Bomb§7 gebruikt!");
	...
	public void handleFireShield(){
		if(item.getType() == Material.FIREBALL && item.getItemMeta().getDisplayName().equals("§f§lFire Shield")){
			...
			if(!omp.onCooldown(Cooldown.FIRE_SHIELD)){
				p.sendMessage("§7Je hebt §f§lFire Shield§7 gebruikt!");
	...
	public void handleIronFist(){
		if(item.getType() == Material.IRON_INGOT && item.getItemMeta().getDisplayName().equals("§f§lIron Fist")){
			...
			if(!omp.onCooldown(Cooldown.IRON_FIST)){
				if(p.getHealth() > 2){
					p.sendMessage("§7Je hebt §f§lIron Fist§7 gebruikt!");
					...
					if(!omp.onCooldown(Cooldown.MESSAGE)){
						p.sendMessage("§7Je hebt te weinig HP om §f§lIron Fist§7 te gebruiken!");
	...
	public void handleBackToHub(){
		if(item.getType() == Material.ENDER_PEARL && item.getItemMeta().getDisplayName().equals("§3§nTerug naar de Hub")){
			e.setCancelled(true);
			omp.updateInventory();
				...
				if(arena.getState() != GameState.CLOSED){
					if(arena.getState() != GameState.ENDING && arena.getState() != GameState.RESTARTING){
						if((arena.getState() == GameState.WAITING || arena.getState() == GameState.STARTING) && arena.getPlayers() == arena.getType().getMaxPlayers()){
							p.sendMessage(arenaname + "§7 is §4§lvol§7!");
						}
						else{
							p.sendMessage(arenaname + "§7 aan het joinen...");
							omp.toMiniGame(arena);
						}
					}
					else{
						p.sendMessage(arenaname + "§7 is aan het §8§lrestarten§7!");
					}
				}
				else{
					p.sendMessage(arenaname + "§7 is §4§ldicht§7!");
				}
			}
		}
	}
			...
			if(sign != null){
				if(ServerData.isServer(Server.SURVIVAL) && !omp.getSurvivalPlayer().getShopSigns().contains(sign) || ServerData.isServer(Server.PRISON) && !omp.getPrisonPlayer().getShopSigns().contains(sign)){
						...
						if(sign.getShopType() == ShopType.BUY){
							if(!sign.isSold()){
								if(ServerData.isServer(Server.SURVIVAL) && omp.getSurvivalPlayer().hasMoney(sign.getPrice()) || ServerData.isServer(Server.PRISON) && omp.getPrisonPlayer().hasGold(sign.getPrice())){
									...
									p.sendMessage("§7Je hebt niet genoeg geld!");
								}
							}
							else{
								p.sendMessage("§7Deze shop is §cuitverkocht§7!");
							}
						}
						else{
							if(sign.canSell()){
								if(ServerData.isServer(Server.SURVIVAL) && sign.hasMoney() || ServerData.isServer(Server.PRISON) && sign.hasGold()){
									...
									p.sendMessage("§7De eigenaar van deze shop heeft niet genoeg geld!");
								}
							}
							else{
								p.sendMessage("§7Je kan hier niet meer verkopen, de chest is §cvol§7!");
							}
						}
					}
					else{}
					...
					if(a == Action.RIGHT_CLICK_BLOCK){
						p.sendMessage("§7Je kan jouw eigen Chest Shop niet gebruiken!");
					}
				}
			}
		}
	...
	public void handleBackToLobby(){
		if(item.getType() == Material.ENDER_PEARL && item.getItemMeta().getDisplayName().equals("§3§nTerug naar de Lobby")){
			e.setCancelled(true);
			omp.updateInventory();
	...
	public void handleChickenPetAge(){
		if(item.getType() == Material.RAW_CHICKEN && item.getItemMeta().getDisplayName().equals("§c§nVerander Leeftijd")){
			e.setCancelled(true);
			omp.updateInventory();
			...
			if(c.isAdult()){
				c.setBaby();
				p.sendMessage("§f" + omp.getPetName(Pet.CHICKEN) +"'s§7 leeftijd veranderd in §7§lBaby§7!");
				item.setAmount(1);
			}
			else{
				c.setAdult();
				p.sendMessage("§f" + omp.getPetName(Pet.CHICKEN)  +"'s§7 leeftijd veranderd in §7§lVolwassen§7!");
				item.setAmount(2);
			}
	...
	public void handleCreeperPetType(){
		if(item.getType() == Material.MONSTER_EGG && item.getItemMeta().getDisplayName().startsWith("§a§nVerander Type")){
			e.setCancelled(true);
			omp.updateInventory();
			...
			if(c.isPowered()){
				...
				meta.setDisplayName("§a§nVerander Type§7 (§6§lNORMAAL§7)");
				item.setItemMeta(meta);
			}
			else{
				c.setPowered(true);
				ItemMeta meta = item.getItemMeta();
				meta.setDisplayName("§a§nVerander Type§7 (§e§lLIGHTNING§7)");
			}
	...
	public void handleCreeperPetExplode(){
		if(item.getType() == Material.TNT && item.getItemMeta().getDisplayName().equals("§c§nExplodeer")){
			e.setCancelled(true);
			omp.updateInventory();
	...
	public void handleOcelotPetColor(){
		if(item.getType() == Material.RAW_FISH && item.getItemMeta().getDisplayName().equals("§9§nVerander Kleur")){
			e.setCancelled(true);
			omp.updateInventory();
	...
	public void handleHorsePetColor(){
		if(item.getType() == Material.LEATHER && item.getItemMeta().getDisplayName().equals("§e§nVerander Kleur")){
			e.setCancelled(true);
			omp.updateInventory();
	...
	public void handleHorsePetSpeed(){
		if(item.getType() == Material.FEATHER && item.getItemMeta().getDisplayName().equals("§f§nVerander Snelheid")){
			e.setCancelled(true);
			omp.updateInventory();
			...
			p.sendMessage("§f" + omp.getPetName(Pet.HORSE) +"'s§7 snelheid veranderd in §6§l" + speed + "§7!");
			p.playSound(p.getLocation(), Sound.CLICK, 5, 1);
		}
	}
	...
	public void handleSilverfishPetBomb(){
		if(item.getType() == Material.MONSTER_EGG && item.getItemMeta().getDisplayName().equals("§7§nSilverfish Bom")){
			e.setCancelled(true);
			omp.updateInventory();
	...
	public void handleSheepPetDisco(){
		if(item.getType() == Material.WOOL && item.getItemMeta().getDisplayName().startsWith("§f§nSheep Disco")){
			...
			p.sendMessage("§fSheep Disco staat nu " + Utils.statusString(!omp.hasPetSheepDisco()) + "§7!");
			p.playSound(p.getLocation(), Sound.CLICK, 5, 1);
	...
	public void handleSheepPetColor(){
		if(item.getType() == Material.INK_SACK && item.getItemMeta().getDisplayName().startsWith("§f§nVerander Kleur")){
			e.setCancelled(true);
			omp.updateInventory();
			...
			ItemMeta meta = item.getItemMeta();
			meta.setDisplayName("§f§nVerander Kleur§7 (" + Utils.getDyeColorName(c) + "§7)");
			item.setItemMeta(meta);
	...
	public void handleCowPetAge(){
		if(item.getType() == Material.RAW_BEEF && item.getItemMeta().getDisplayName().equals("§c§nVerander Leeftijd")){
			e.setCancelled(true);
			omp.updateInventory();
			...
			if(cow.isAdult()){
				cow.setBaby();
				p.sendMessage("§f" + omp.getPetName(Pet.COW) +"'s§7 leeftijd veranderd in §8§lBaby§7!");
				item.setAmount(1);
			}
			else{
				cow.setAdult();
				p.sendMessage("§f" + omp.getPetName(Pet.COW) +"'s§7 leeftijd veranderd in §8§lVolwassen§7!");
				item.setAmount(2);
			}
	...
	public void handleWolfPetAge(){
		if(item.getType() == Material.BONE && item.getItemMeta().getDisplayName().equals("§7§nVerander Leeftijd")){
			e.setCancelled(true);
			omp.updateInventory();
			...
			if(wolf.isAdult()){
				wolf.setBaby();
				p.sendMessage("§f" + omp.getPetName(Pet.WOLF) +"'s§7 leeftijd veranderd in §7§lBaby§7!");
				item.setAmount(1);
			}
			else{
				wolf.setAdult();
				p.sendMessage("§f" + omp.getPetName(Pet.WOLF) +"'s§7 leeftijd veranderd in §7§lVolwassen§7!");
				item.setAmount(2);
			}
	...
	public void handleSlimePetSize(){
		if(item.getType() == Material.SLIME_BALL && item.getItemMeta().getDisplayName().equals("§a§nVerander Grootte")){
			e.setCancelled(true);
			omp.updateInventory();
			...
			p.sendMessage("§f" + omp.getPetName(Pet.SLIME) +"'s§7 grootte veranderd in §a§l" + size + "§7!");
			p.playSound(p.getLocation(), Sound.CLICK, 5, 1);
		}
	}
	...
	public void handlePetPigBabies(){
		if(item.getType() == Material.MONSTER_EGG && item.getItemMeta().getDisplayName().equals("§d§nBaby Pigs§7 (" + Utils.statusString(omp.hasPetBabyPigs()) + "§7)")){
			...
			p.sendMessage("§dBaby Pigs§7 staan nu " + Utils.statusString(!omp.hasPetBabyPigs()) + "§7!");
			p.playSound(p.getLocation(), Sound.CLICK, 5, 1);
	...
	public void handlePetPigAge(){
		if(item.getType() == Material.PORK && item.getItemMeta().getDisplayName().equals("§d§nVerander Leeftijd")){
			e.setCancelled(true);
			omp.updateInventory();
			...
			if(pig.isAdult()){
				pig.setBaby();
				p.sendMessage("§f" + omp.getPetName(Pet.PIG) + "'s§7 leeftijd veranderd in §d§lBaby§7!");
				item.setAmount(1);
			}
			else{
				pig.setAdult();
				p.sendMessage("§f" + omp.getPetName(Pet.PIG)  + "'s§7 leeftijd veranderd in §d§lAdult§7!");
				item.setAmount(2);
			}
	...
	public void handleMagmaCubePetSize(){
		if(item.getType() == Material.MAGMA_CREAM && item.getItemMeta().getDisplayName().equals("§c§nVerander Grootte")){
			e.setCancelled(true);
			omp.updateInventory();
			...
			p.sendMessage("§f" + omp.getPetName(Pet.MAGMA_CUBE) + "'s§7 grootte veranderd in §c§l" + size + "§7!");
			p.playSound(p.getLocation(), Sound.CLICK, 5, 1);
		}
	}
	...
	public void handleMushroomCowPetShroomTrail(){
		if(item.getType() == Material.HUGE_MUSHROOM_1 || item.getType() == Material.HUGE_MUSHROOM_2){
			if(item.getItemMeta().getDisplayName().equals("§4§nShroom Trail§7 (" + Utils.statusString(omp.hasPetShroomTrail()) + "§7)")){
				...
				p.sendMessage("§7De §4Shroom Trail§7 staat nu " + Utils.statusString(!omp.hasPetShroomTrail()) + "§7!");
				p.playSound(p.getLocation(), Sound.CLICK, 5, 1);
	...
	public void handleMagmaCubeSoccer(){
		if(item.getType() == Material.MAGMA_CREAM && item.getItemMeta().getDisplayName().equals("§c§nMagmaCube Voetbal")){
			if(omp.getSoccerMagmaCube() == null){
				...
				mc.setCustomName("§cVoetbal");
				mc.setCustomNameVisible(true);
				
				ServerStorage.soccermagmacubes.add(mc);
				omp.setSoccerMagmaCube(mc);
				
				p.sendMessage("§cMagmaCube Voetbal§7 staat nu §a§lAAN§7. §eRechtermuisknop§7 om het uit te zetten. §eLinkermuisknop§7 om het te schieten.");
				p.getWorld().playEffect(p.getLocation(), Effect.STEP_SOUND, 152);
			}
			else{
				omp.getSoccerMagmaCube().teleport(p.getLocation());
				p.sendMessage("§7Jouw §cMagmaCube Voetbal§7 is naar jou toe geteleporteerd!");
				p.getWorld().playEffect(p.getLocation(), Effect.STEP_SOUND, 152);
			}
		}
	}
	...
	public void handleSnowGolemAttack(){
		if(item.getType() == Material.PUMPKIN && item.getItemMeta().getDisplayName().equals("§6§nSnowman Attack")){
			...
			if(!omp.onCooldown(Cooldown.SGA_USAGE)){
				...
				Bukkit.broadcastMessage(omp.getName() + "§7 heeft een §6§lSnowman Attack§7 gespawned!");
				for(Player player : Bukkit.getOnlinePlayers()){
	...
	public void handleFireworkGun(){
		if(item.getType() == Material.FIREBALL && item.getItemMeta().getDisplayName().startsWith("§c§nFirework Gun")){
			...
			if(omp.getFireworkPasses() != 0){
				...
				p.sendMessage("§7Je hebt geen §6§lFirework Passes§7 meer!");
			}
		}
	}
	...
	public void handleAchievements(){
		if(item.getType() == Material.EXP_BOTTLE && item.getItemMeta().getDisplayName().equals("§d§nAchievements")){
			...
			p.sendMessage("§a§oBinnenkort beschikbaar...");
	...
	public void handleMindCraft(){
		if(omp.isInMindCraft()){
			...
			if(item != null && item.getItemMeta() != null && item.getItemMeta().getDisplayName() != null){
				if(item.getType() == Material.TNT && item.getItemMeta().getDisplayName().equals("§4§lReset Kleuren")){
					e.setCancelled(true);
					omp.updateInventory();
				...
				if(item.getType() == Material.REDSTONE_TORCH_ON && item.getItemMeta().getDisplayName().equals("§c§lProbeer Kleurcode")){
					e.setCancelled(true);
					omp.updateInventory();
					...
					if(!newturnnext.contains("0")){
						...
						if(mcp.getCorrectTurn().equals(newturnnext)){
							...
							p.sendMessage("§c§lMindCraft §8| §7Je hebt de goede kleurcode geraden!");
							...
							Title t = new Title("§c§lMindCraft", "§7Je hebt kleurcode geraden! §c+1 Win");
							t.send(p);
							...
							p.sendMessage("§c§lMindCraft §8| §7Je hebt §cverloren§7. Probeer het opnieuw!");
							
							Title t = new Title("§c§lMindCraft", "§7Je hebt verloren. Probeer het opnieuw!");
							t.send(p);
							...
							p.playSound(p.getLocation(), Sound.CLICK, 5, 1);
							p.sendMessage("§c§lMindCraft §8| §7Goed: §a" + correct + " §7Andere Plek: §e" + otherplace + " §7Fout: §c" + incorrect);
						}
					}
					else{
						p.sendMessage("§c§lMindCraft §8| §7Jouw kleurcode is nog niet klaar!");
					}
				}
			}
			if(a == Action.RIGHT_CLICK_BLOCK){
		...
		if(item.getItemMeta().getLore() != null){
			...
			if(a == Action.RIGHT_CLICK_AIR || a == Action.RIGHT_CLICK_BLOCK){
				...
				if(lore.contains(ItemType.PAINTBALLS_I.getName())){
					...
					if(name.endsWith("Rood §8|| §bWapen§f")){
						name = name.replace("§c§lRood", "§b§lBlauw");
						color = org.bukkit.Color.AQUA;
					}
					else if(name.endsWith("Blauw §8|| §bWapen§f")){
						name = name.replace("§b§lBlauw", "§0§lZwart");
						color = org.bukkit.Color.BLACK;
					}
					else if(name.endsWith("Zwart §8|| §bWapen§f")){
						name = name.replace("§0§lZwart", "§7§lGrijs");
						color = org.bukkit.Color.SILVER;
					}
					else if(name.endsWith("Grijs §8|| §bWapen§f")){
						name = name.replace("§7§lGrijs", "§a§lGroen");
						color = org.bukkit.Color.LIME;
					}
					else if(name.endsWith("Groen §8|| §bWapen§f")){
						name = name.replace("§a§lGroen", "§c§lRood");
