...
public class CreativePlayer {
	...
	public void selectKit(Kit kit){
		...
		Title t = new Title("", "§7Kit Geselecteerd: '§d§l" + kit.getKitName() + "§7'");
		t.send(this.player);
	}
	...
	public boolean isOnPlot(Location l){
		...
		if(!onplot){
			...
			if(!omp.onCooldown(Cooldown.MESSAGE)){
				this.player.sendMessage("§7Je kan dat hier niet doen!");
		...
		new BukkitRunnable(){
			public void run(){
				...
				for(ItemStack item : armorcontents){
					if(item != null && item.getType() == Material.STAINED_GLASS_PANE && item.getItemMeta() != null && item.getItemMeta().getDisplayName().endsWith("§7§lLeeg")){
