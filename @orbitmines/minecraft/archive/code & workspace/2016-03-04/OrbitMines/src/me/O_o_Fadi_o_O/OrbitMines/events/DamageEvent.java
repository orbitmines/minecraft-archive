		...
		if(ServerData.isServer(Server.HUB, Server.MINIGAMES)){
			...
			if(e.getEntity() instanceof Player && e.getDamager() instanceof Player){
				...
				if(i != null && i.getType() == Material.LEASH && i.getItemMeta().getDisplayName().equals("§6§nStacker")){
					...
					Language laE = ompE.getLanguage();
					Language laD = ompD.getLanguage();
							...
							if(ompE.hasStackerEnabled()){
								if(ompD.hasStackerEnabled()){
									if(ompE.hasPlayersEnabled()){
										...
										Language.Message.STACKED.send(laD, pD, ompE.getName());
										pD.playEffect(pD.getLocation(), Effect.STEP_SOUND, 152);
                                        Language.Message.BEEN_STACKED.send(laE, pE, ompD.getName());
										e.setCancelled(true);
								    }
								    else{
                                        Language.Message.PLAYERS_OFF.send(laD, pD, ompE.getName());
                                    }
                                }
                                else{
                                    Language.Message.YOU_STACKING_OFF.send(laD, pD);
                                }
                            }
                            else{
                                Language.Message.STACKING_OFF.send(laD, pD, ompE.getName());
                            }
                        }
                    }
                }
		...
		if(ServerData.isServer(Server.KITPVP)){
			if(e.getDamager() instanceof Player){
				...
				if(e.getEntity() instanceof Player){
					...
					if(kpE.getSummonedUndeath().size() > 0){
						for(Entity en : kpE.getSummonedUndeath()){
					...
					for(ItemStack item : pE.getInventory().getArmorContents()){
						if(item != null && item.getItemMeta() != null && item.getItemMeta().getLore() != null){
							...
							for(ArmorType type : ArmorType.getArmorTypes(kpE.getKitSelected())){
					...
					if(item != null && item.getItemMeta() != null && item.getItemMeta().getLore() != null){
						...
						for(ItemType type : ItemType.getItemTypes(kpE.getKitSelected())){
				...
				for(OMPlayer omp : OMPlayer.getOMPlayers()){
					if(omp.getKitPvPPlayer().getSummonedUndeath().contains(e.getDamager())){
						ompD = omp;
					}
				}
				...
				if(proj.getShooter() instanceof Player){
					...
					OMPlayer ompD = OMPlayer.getOMPlayer(pD);
					Masteries mD = ompD.getKitPvPPlayer().getMasteries();
					...
					if(kitpvp.isProjectile(proj)){
						ProjectileType.playEnchantment(ompD, ompE, proj);
					...
					if(arena.getSpectators().contains(ompD)){
						...
						if(arena.getState() != GameState.IN_GAME){
							...
							if(kitD != null && kitD.getKitName().equals(TicketType.RUNNER_KIT.getKit().getKitName())){
								e.setCancelled(true);
                                Language.Message.YOU_RUNNER_KIT.send(laD, pD, TicketType.RUNNER_KIT.getName());
							}
								...
								if(kitE != null && kitE.getKitName().equals(TicketType.RUNNER_KIT.getKit().getKitName())){
									e.setCancelled(true);
                                    Language.Message.RUNNER_KIT.send(laD, pD, ompE.getName(), TicketType.RUNNER_KIT.getName());
								}
							}
							...
							if(e.getEntity() instanceof Player){
								...
								if(!arena.getGA().isGhost(ompD) && !arena.getGA().isGhost(ompE)){
									e.setCancelled(true);
								}
								else{
									if(arena.getGA().isGhost(ompE)){
										arena.getGA().setRevealedIn(1);
									}
								}
							}
						}
