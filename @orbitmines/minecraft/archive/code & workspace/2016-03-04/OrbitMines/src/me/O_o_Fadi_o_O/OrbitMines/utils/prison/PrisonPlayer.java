...
public class PrisonPlayer {
	...
	public void rankup(){
		...
		Bukkit.broadcastMessage(OMPlayer.getOMPlayer(player).getName() + "§7 heeft een rankup gedaan naar §a§l" + nextrank.toString() + "§7!");
		
		if(nextrank.getNextRank() == null){
			getPlayer().sendMessage("§7Je hebt de hoogste rank bereikt! Meer ranks komen binnenkort!");
		}
	...
	public void setClockEnabled(boolean clockenabled) {
		...
		getPlayer().sendMessage("§7Clock staat nu " + Utils.statusString(clockenabled) + "§7!");
	...
	public boolean inMine(Location l, boolean message){
		...
		if(!inmine && message){
			...
			if(!omp.onCooldown(Cooldown.MESSAGE)){
				getPlayer().sendMessage("§7Je kan dat hier niet doen!");
	...
	public void requiredGold(int gold){
		...
		getPlayer().sendMessage("§7Je hebt nog §6§l" + required + " §6§lGold§7 nodig.");
	}
	public void requiredGambleTickets(int tickets){
		...
		if(needed == 1){
			p.sendMessage("§7Je hebt nog §a" + needed + " §aGamble Ticket§7 nodig!");
		}
		else{
			p.sendMessage("§7Je hebt nog §a" + needed + " §aGamble Tickets§7 nodig!");
		}
	}
