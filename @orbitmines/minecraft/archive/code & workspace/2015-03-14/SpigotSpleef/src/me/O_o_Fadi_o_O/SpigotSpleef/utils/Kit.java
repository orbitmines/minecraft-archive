...
public class Kit {
	...
	public ItemStack getReplacedItemStack(SpleefPlayer sp, KitItemStack item, int index, String animatedrightclick){
		if(item.getItemStack().getType() != Material.AIR){
			...
			//Check Abilities
			if(item.containsAbility(Ability.INSTANT_TNT)){
				
			}
			else if(item.containsAbility(Ability.JUMP)){
				long last = 0;
			...
			if(item.containsAbility(Ability.PLUS_ONE_1)){
				newi = AbilityManager.handlePlusOne(Ability.PLUS_ONE_1, sp, item, index, newi, meta);
				if(newi != null && newi.getItemMeta() != null){
					meta = newi.getItemMeta();
				}
			}
			else if(item.containsAbility(Ability.PLUS_ONE_2)){
				newi = AbilityManager.handlePlusOne(Ability.PLUS_ONE_2, sp, item, index, newi, meta);
				if(newi != null && newi.getItemMeta() != null){
					meta = newi.getItemMeta();
				}
			}
			else if(item.containsAbility(Ability.PLUS_ONE_3)){
				newi = AbilityManager.handlePlusOne(Ability.PLUS_ONE_3, sp, item, index, newi, meta);
				if(newi != null && newi.getItemMeta() != null){
					meta = newi.getItemMeta();
				}
			}
			else if(item.containsAbility(Ability.PLUS_ONE_4)){
				newi = AbilityManager.handlePlusOne(Ability.PLUS_ONE_4, sp, item, index, newi, meta);
				if(newi != null && newi.getItemMeta() != null){
					meta = newi.getItemMeta();
				}
			}
			else if(item.containsAbility(Ability.PLUS_ONE_5)){
				newi = AbilityManager.handlePlusOne(Ability.PLUS_ONE_5, sp, item, index, newi, meta);
				if(newi != null && newi.getItemMeta() != null){
					meta = newi.getItemMeta();
				}
			}
