...
public class ShopSign {
	...
	public void buyItems(OMPlayer omp){
		...
		for(ItemStack item : items){
			if(amount != this.amount){
				if(amount + item.getAmount() < this.amount || amount + item.getAmount() == this.amount){
		...
		if(p2 != null){
			...
			p.sendMessage("§7You've bought §6" + new SurvivalUtils().getMaterialName(m) + "§7(§6" + this.amount + "§7) from §6" + p2.getName() + " §7for §6" + this.price + "$§7.");
			p2.sendMessage("§6" + p.getName() + "§7 bought §6" + new SurvivalUtils().getMaterialName(m) + "§7(§6" + this.amount + "x§7) from you for §6" + this.price + "$§7.");
		}
		else{
			p.sendMessage("§7You've bought §6" + new SurvivalUtils().getMaterialName(m) + "§7(§6" + this.amount + "§7) from §6" + Utils.getName(this.uuid) + " §7for §6" + this.price + "$§7.");
	...
	public void sellItems(OMPlayer omp){
		...
		if(p2 != null){
			...
			omp.getPlayer().sendMessage("§7You've sold §6" + new SurvivalUtils().getMaterialName(m) + "§7(§6" + this.amount + "§7) to §6" + p2.getName() + " §7for §6" + this.price + "$§7.");
			p2.sendMessage("§6" + omp.getPlayer().getName() + "§7 sold §6" + new SurvivalUtils().getMaterialName(m) + "§7(§6" + this.amount + "x§7) to you for §6" + this.price + "$§7.");
		}
		else{
			omp.getPlayer().sendMessage("§7You've sold §6" + new SurvivalUtils().getMaterialName(m) + "§7(§6" + this.amount + "§7) to §6" + Utils.getName(this.uuid) + " §7for §6" + this.price + "$§7.");
			Database.get().update("Survival-Money", "money", "" + (getMoneyOffline() - this.price), "uuid", this.uuid.toString());
		}
	}
	...
	public void update(){
		...
		if(b != null && (b.getType() == Material.SIGN_POST || b.getType() == Material.WALL_SIGN)){
			...
			lines[2] = this.amount + " : " + this.price + "$";
	...
	public void delete(){
		...
		ServerData.getSurvival().getShopSigns().remove(this);
		saveToConfig();
	}
	...
	public static List<ShopSign> getShopSigns(Player player){
		...
		for(ShopSign sign : ServerData.getSurvival().getShopSigns()){
